...
Read more: https://mswjs.io/docs/http/intercepting-requests`}async resolve(e,r,s){const{id:n,request:o}=this.data,i=s?.quiet?null:o.clone();if(this.events.emit(new W("request:start",{requestId:n,request:o})),Tr(o))return this.events.emit(new W("request:end",{requestId:n,request:o})),this.passthrough(),null;const[a,h]=await jr(()=>Rr({requestId:n,request:o,handlers:e,resolutionContext:{baseUrl:s?.baseUrl?.toString(),quiet:s?.quiet}}));if(a!=null)return this.events.emit(new Ar("unhandledException",{error:a,requestId:n,request:o}))||(console.error(a),v.error('Encountered an unhandled exception during the handler lookup for "%s %s". Please see the original error above.',o.method,o.url)),this.errorWith(a),null;if(h==null)return this.events.emit(new W("request:unhandled",{requestId:n,request:o})),await Me(this,r).then(()=>this.passthrough(),y=>this.errorWith(y)),this.events.emit(new W("request:end",{requestId:n,request:o})),!1;const{response:l,handler:m,parsedResult:f}=h;if(this.events.emit(new W("request:match",{requestId:n,request:o})),l==null)return this.events.emit(new W("request:end",{requestId:n,request:o})),this.passthrough(),null;if(Nr(l))return this.events.emit(new W("request:end",{requestId:n,request:o})),this.passthrough(),null;const p=s?.quiet?null:l.clone();return await Pr(o,l),this.respondWith(l),this.events.emit(new W("request:end",{requestId:n,request:o})),s?.quiet||m.log({request:i,response:p,parsedResult:f}),!0}}async function Me(t,e){const r=async n=>{if(n==="bypass")return;const o=await t.getUnhandledMessage();switch(n){case"warn":return v.warn("Warning: %s",o);case"error":return v.error("Error: %s",o)}},s=async n=>{if(_.as(Je,n==="bypass"||n==="warn"||n==="error",v.formatMessage('Failed to react to an unhandled network frame: unknown strategy "%s". Please provide one of the supported strategies ("bypass", "warn", "error") or a custom callback function as the value of the "onUnhandledRequest" option.',n)),n!=="bypass"&&(await r(n),n==="error"))return Promise.reject(new Je(v.formatMessage('Cannot bypass a request when using the "error" strategy for the "onUnhandledRequest" option.')))};if(typeof e=="function")return e({frame:t,defaults:{warn:r.bind(null,"warn"),error:r.bind(null,"error")}});if(!(t instanceof Ie&&Mr(t.data.request)))return s(e)}function ht(t){const e=[];for(const r of t)r instanceof Promise&&e.push(r);if(e.length>0)return Promise.all(e).then(()=>{})}var Ke=(t=>(t[t.DISABLED=0]="DISABLED",t[t.ENABLED=1]="ENABLED",t))(Ke||{});function Dr(t){let e=0;const r=new et,s=new qr,n=a=>a instanceof Gt?a:new _r(a||[]);let o={...t},i=n(o.handlers);return{get readyState(){return e},events:r,configure(a){_(e===0,'Failed to call "configure()" on the network: cannot configure an already enabled network.'),a.handlers&&!Object.is(a.handlers,o.handlers)&&(i=n(a.handlers)),o={...o,...a}},enable(){_(e===0,'Failed to call "enable" on the network: already enabled'),e=1;const a={active:!0};s.subscriptions.push(()=>{a.active=!1});const h=o.sources.map(l=>(tt.prototype.disable.call(l),l.on("frame",async({frame:m})=>{m.events.on("*",p=>{a.active&&r.emit(p)});const f=m.getHandlers(i);await m.resolve(f,o.onUnhandledFrame||"warn",o.context)}),l.enable()));return ht(h)},disable(){return _(e===1,'Failed to call "disable" on the network: already disabled'),e=0,s.dispose(),ht(o.sources.map(a=>a.disable()))},use(...a){i.use(a)},resetHandlers(...a){i.reset(a)},restoreHandlers(){i.restore()},listHandlers(){return Cr(i.currentHandlers())}}}async function Gr(t,e,...r){const s=t.listeners(e);if(s.length!==0)for(const n of s)await n.apply(t,r)}var Hr=class{#e=new Map;applyPatch(t,e,r){const s=this.#e.get(t);_(!s?.has(e),`Failed to replace a global value at "${String(e)}": already replaced.`);const n=$t(t,e);if(typeof n>"u")return console.warn(`Failed to replace a global value at "${String(e)}": not a global value.`),()=>{};if(n.descriptor.configurable)Object.defineProperty(t,e,{value:r(t[e]),enumerable:!0,configurable:!0});else if(n.descriptor.writable)t[e]=r(t[e]);else throw new Error(`Failed to patch a non-configurable non-writable property "${e.toString()}"`);const o=()=>{const i=this.#e.get(t);i?.has(e)&&(n.owner===t?Object.defineProperty(n.owner,e,n.descriptor):Reflect.deleteProperty(t,e),i.delete(e),i.size===0&&this.#e.delete(t))};return s?s.set(e,o):this.#e.set(t,new Map([[e,o]])),o}restoreAllPatches(){const t=[];for(const[,e]of this.#e)for(const[,r]of e)try{r()}catch(s){if(s instanceof Error)t.push(s);else throw s}if(t.length>0)throw new AggregateError(t,"FOO!")}};const Ur=new Hr;function $t(t,e){let r=t,s;for(;r;){if(s=Object.getOwnPropertyDescriptor(r,e),s)return{owner:r,descriptor:s};r=Object.getPrototypeOf(r)}}function $r(t){const e=$t(globalThis,t);if(typeof e>"u")return!1;const{descriptor:r}=e;return typeof r.get=="function"&&typeof r.get()>"u"||typeof r.get>"u"&&r.value==null?!1:typeof r.set>"u"&&!r.configurable?(console.error(`[MSW] Failed to apply interceptor: the global \`${t}\` property is non-configurable. This is likely an issue with your environment. If you are using a framework, please open an issue about this in their repository.`),!1):!0}function L(t,e){return Object.defineProperties(e,{target:{value:t,enumerable:!0,writable:!0},currentTarget:{value:t,enumerable:!0,writable:!0}}),e}const ee=Symbol("kCancelable"),I=Symbol("kDefaultPrevented");var rt=class extends MessageEvent{constructor(e,r){super(e,r),this[ee]=!!r.cancelable,this[I]=!1}get cancelable(){return this[ee]}set cancelable(e){this[ee]=e}get defaultPrevented(){return this[I]}set defaultPrevented(e){this[I]=e}preventDefault(){this.cancelable&&!this[I]&&(this[I]=!0)}},Te=class extends Event{constructor(e,r={}){super(e,r),this.code=r.code===void 0?0:r.code,this.reason=r.reason===void 0?"":r.reason,this.wasClean=r.wasClean===void 0?!1:r.wasClean}},pt=class extends Te{constructor(e,r={}){super(e,r),this[ee]=!!r.cancelable,this[I]=!1}get cancelable(){return this[ee]}set cancelable(e){this[ee]=e}get defaultPrevented(){return this[I]}set defaultPrevented(e){this[I]=e}preventDefault(){this.cancelable&&!this[I]&&(this[I]=!0)}};const ne=Symbol("kEmitter"),we=Symbol("kBoundListener");var Br=class{constructor(e,r){this.socket=e,this.transport=r,this.id=At(),this.url=new URL(e.url),this[ne]=new EventTarget,this.transport.addEventListener("outgoing",s=>{const n=L(this.socket,new rt("message",{data:s.data,origin:s.origin,cancelable:!0}));this[ne].dispatchEvent(n),n.defaultPrevented&&s.preventDefault()}),this.transport.addEventListener("close",s=>{this[ne].dispatchEvent(L(this.socket,new Te("close",s)))})}addEventListener(e,r,s){if(!Reflect.has(r,we)){const n=r.bind(this.socket);Object.defineProperty(r,we,{value:n,enumerable:!1,configurable:!1})}this[ne].addEventListener(e,Reflect.get(r,we),s)}removeEventListener(e,r,s){this[ne].removeEventListener(e,Reflect.get(r,we),s)}send(e){this.transport.send(e)}close(e,r){this.transport.close(e,r)}};const ft="InvalidAccessError: close code out of user configurable range",Pe=Symbol("kPassthroughPromise"),Bt=Symbol("kOnSend"),pe=Symbol("kClose");var Fr=class extends EventTarget{static{this.CONNECTING=0}static{this.OPEN=1}static{this.CLOSING=2}static{this.CLOSED=3}constructor(e,r){super(),this.CONNECTING=0,this.OPEN=1,this.CLOSING=2,this.CLOSED=3,this._onopen=null,this._onmessage=null,this._onerror=null,this._onclose=null,this.url=Dt(e),this.protocol="",this.extensions="",this.binaryType="blob",this.readyState=this.CONNECTING,this.bufferedAmount=0,this[Pe]=new vr,queueMicrotask(async()=>{await this[Pe]||(this.protocol=typeof r=="string"?r:Array.isArray(r)&&r.length>0?r[0]:"",this.readyState===this.CONNECTING&&(this.readyState=this.OPEN,this.dispatchEvent(L(this,new Event("open")))))})}set onopen(e){this.removeEventListener("open",this._onopen),this._onopen=e,e!==null&&this.addEventListener("open",e)}get onopen(){return this._onopen}set onmessage(e){this.removeEventListener("message",this._onmessage),this._onmessage=e,e!==null&&this.addEventListener("message",e)}get onmessage(){return this._onmessage}set onerror(e){this.removeEventListener("error",this._onerror),this._onerror=e,e!==null&&this.addEventListener("error",e)}get onerror(){return this._onerror}set onclose(e){this.removeEventListener("close",this._onclose),this._onclose=e,e!==null&&this.addEventListener("close",e)}get onclose(){return this._onclose}send(e){if(this.readyState===this.CONNECTING)throw this.close(),new DOMException("InvalidStateError");this.readyState===this.CLOSING||this.readyState===this.CLOSED||(this.bufferedAmount+=Qr(e),queueMicrotask(()=>{this.bufferedAmount=0,this[Bt]?.(e)}))}close(e=1e3,r){_(e,ft),_(e===1e3||e>=3e3&&e<=4999,ft),this[pe](e,r)}[pe](e=1e3,r,s=!0){this.readyState===this.CLOSING||this.readyState===this.CLOSED||(this.readyState=this.CLOSING,queueMicrotask(()=>{this.readyState=this.CLOSED,this.dispatchEvent(L(this,new Te("close",{code:e,reason:r,wasClean:s}))),this._onopen=null,this._onmessage=null,this._onerror=null,this._onclose=null}))}addEventListener(e,r,s){return super.addEventListener(e,r,s)}removeEventListener(e,r,s){return super.removeEventListener(e,r,s)}};function Qr(t){return typeof t=="string"?t.length:t instanceof Blob?t.size:t.byteLength}const A=Symbol("kEmitter"),ve=Symbol("kBoundListener"),Ge=Symbol("kSend");var zr=class{constructor(e,r,s){this.client=e,this.transport=r,this.createConnection=s,this[A]=new EventTarget,this.mockCloseController=new AbortController,this.realCloseController=new AbortController,this.transport.addEventListener("outgoing",n=>{typeof this.realWebSocket>"u"||queueMicrotask(()=>{n.defaultPrevented||this[Ge](n.data)})}),this.transport.addEventListener("incoming",this.handleIncomingMessage.bind(this))}get socket(){return _(this.realWebSocket,'Cannot access "socket" on the original WebSocket server object: the connection is not open. Did you forget to call `server.connect()`?'),this.realWebSocket}connect(){_(!this.realWebSocket||this.realWebSocket.readyState!==WebSocket.OPEN,'Failed to call "connect()" on the original WebSocket instance: the connection already open');const e=this.createConnection();e.binaryType=this.client.binaryType,e.addEventListener("open",r=>{this[A].dispatchEvent(L(this.realWebSocket,new Event("open",r)))},{once:!0}),e.addEventListener("message",r=>{this.transport.dispatchEvent(L(this.realWebSocket,new MessageEvent("incoming",{data:r.data,origin:r.origin})))}),this.client.addEventListener("close",r=>{this.handleMockClose(r)},{signal:this.mockCloseController.signal}),e.addEventListener("close",r=>{this.handleRealClose(r)},{signal:this.realCloseController.signal}),e.addEventListener("error",()=>{const r=L(e,new Event("error",{cancelable:!0}));this[A].dispatchEvent(r),r.defaultPrevented||this.client.dispatchEvent(L(this.client,new Event("error")))}),this.realWebSocket=e}addEventListener(e,r,s){if(!Reflect.has(r,ve)){const n=r.bind(this.client);Object.defineProperty(r,ve,{value:n,enumerable:!1})}this[A].addEventListener(e,Reflect.get(r,ve),s)}removeEventListener(e,r,s){this[A].removeEventListener(e,Reflect.get(r,ve),s)}send(e){this[Ge](e)}[Ge](e){const{realWebSocket:r}=this;if(_(r,'Failed to call "server.send()" for "%s": the connection is not open. Did you forget to call "server.connect()"?',this.client.url),!(r.readyState===WebSocket.CLOSING||r.readyState===WebSocket.CLOSED)){if(r.readyState===WebSocket.CONNECTING){r.addEventListener("open",()=>{r.send(e)},{once:!0});return}r.send(e)}}close(){const{realWebSocket:e}=this;_(e,'Failed to close server connection for "%s": the connection is not open. Did you forget to call "server.connect()"?',this.client.url),this.realCloseController.abort(),!(e.readyState===WebSocket.CLOSING||e.readyState===WebSocket.CLOSED)&&(e.close(),queueMicrotask(()=>{this[A].dispatchEvent(L(this.realWebSocket,new pt("close",{code:1e3,cancelable:!0})))}))}handleIncomingMessage(e){const r=L(e.target,new rt("message",{data:e.data,origin:e.origin,cancelable:!0}));this[A].dispatchEvent(r),r.defaultPrevented||this.client.dispatchEvent(L(this.client,new MessageEvent("message",{data:e.data,origin:e.origin})))}handleMockClose(e){this.realWebSocket&&this.realWebSocket.close()}handleRealClose(e){this.mockCloseController.abort();const r=L(this.realWebSocket,new pt("close",{code:e.code,reason:e.reason,wasClean:e.wasClean,cancelable:!0}));this[A].dispatchEvent(r),r.defaultPrevented||this.client[pe](e.code,e.reason)}},Zr=class extends EventTarget{constructor(e){super(),this.socket=e,this.socket.addEventListener("close",r=>{this.dispatchEvent(L(this.socket,new Te("close",r)))}),this.socket[Bt]=r=>{this.dispatchEvent(L(this.socket,new rt("outgoing",{data:r,origin:this.socket.url,cancelable:!0})))}}addEventListener(e,r,s){return super.addEventListener(e,r,s)}dispatchEvent(e){return super.dispatchEvent(e)}send(e){queueMicrotask(()=>{if(this.socket.readyState===this.socket.CLOSING||this.socket.readyState===this.socket.CLOSED)return;const r=()=>{this.socket.dispatchEvent(L(this.socket,new MessageEvent("message",{data:e,origin:this.socket.url})))};this.socket.readyState===this.socket.CONNECTING?this.socket.addEventListener("open",()=>{r()},{once:!0}):r()})}close(e,r){this.socket[pe](e,r)}};(class Ft extends gr{static{this.symbol=Symbol.for("websocket-interceptor")}constructor(){super(Ft.symbol)}checkEnvironment(){return $r("WebSocket")}setup(){const e=this.logger.extend("setup"),r=new Proxy(globalThis.WebSocket,{construct:(s,n,o)=>{const[i,a]=n,h=()=>Reflect.construct(s,n,o),l=new Fr(i,a),m=new Zr(l);return queueMicrotask(async()=>{try{const f=new zr(l,m,h),p=this.emitter.listenerCount("connection")>0;await Gr(this.emitter,"connection",{client:new Br(l,m),server:f,info:{protocols:a}}),p?l[Pe].resolve(!1):(l[Pe].resolve(!0),f.connect(),f.addEventListener("open",()=>{l.dispatchEvent(L(l,new Event("open"))),f.realWebSocket&&(l.protocol=f.realWebSocket.protocol)}))}catch(f){f instanceof Error&&(l.dispatchEvent(new Event("error")),l.readyState!==WebSocket.CLOSING&&l.readyState!==WebSocket.CLOSED&&l[pe](1011,f.message,!1),console.error(f))}}),l}});e.info("patching global WebSocket..."),this.subscriptions.push(Ur.applyPatch(globalThis,"WebSocket",()=>r)),e.info("global WebSocket patched!",globalThis.WebSocket.name)}});class Yr extends se{url;protocols;constructor(e,r){super(e,{}),this.url=r.url,this.protocols=r.protocols}}class Jr extends se{url;protocols;error;constructor(e,r){super(e,{}),this.url=r.url,this.protocols=r.protocols,this.error=r.error}}class Qt extends Ht{constructor(e){super("ws",{connection:e.connection})}getHandlers(e){return e.getHandlersByKind("websocket")}async resolve(e,r,s){const{connection:n}=this.data;if(this.events.emit(new Yr("connection",{url:n.client.url,protocols:n.info.protocols})),e.length===0)return await Me(this,r).then(()=>this.passthrough(),i=>this.errorWith(i)),!1;let o=!1;for(const i of e){const a=await i.run(n,{baseUrl:s?.baseUrl?.toString(),[Lr]:!1});if(!a)continue;o=!0;const h=s?.quiet?void 0:i.log(n);try{i[xr](a)||h?.()}catch(l){throw this.events.emit(new Jr("unhandledException",{error:l,url:n.client.url,protocols:n.info.protocols}))||(console.error(l),v.error('Encountered an unhandled exception during the handler lookup for "%s". Please see the original error above.',n.client.url)),l}}return o?!0:(await Me(this,r).then(()=>this.passthrough(),i=>this.errorWith(i)),!1)}async getUnhandledMessage(){const{connection:e}=this.data;return`intercepted a WebSocket connection without a matching event handler:${`

  • ${e.client.url}

`}If you still wish to intercept this unhandled connection, please create an event handler for it.
Read more: https://mswjs.io/docs/websocket`}}class zt extends tt{#e;#t;constructor(e){super(),this.#e=new yr({name:"interceptor-source",interceptors:e.interceptors}),this.#t=new Map}enable(){this.#e.apply(),this.#e.on("request",this.#s.bind(this)).on("response",this.#n.bind(this)).on("connection",this.#r.bind(this))}disable(){super.disable(),this.#e.dispose(),this.#t.clear()}async#s({requestId:e,request:r,controller:s}){const n=new Kr({id:e,request:r,controller:s});this.#t.set(e,n),await this.queue(n)}async#n({requestId:e,request:r,response:s,isMockedResponse:n}){const o=this.#t.get(e);this.#t.delete(e),o!=null&&queueMicrotask(()=>{try{o.events.emit(new Ut(n?"response:mocked":"response:bypass",{requestId:e,request:r,response:s}))}finally{o.events.removeAllListeners()}})}async#r(e){await this.queue(new Vr({connection:e}))}}class Kr extends Ie{#e;constructor(e){super({id:e.id,request:e.request}),this.#e=e.controller}passthrough(){Wr(this.data.request)}respondWith(e){e&&this.#e.respondWith(e)}errorWith(e){if(e instanceof Response)return this.respondWith(e);throw e instanceof Je&&this.#e.errorWith(e),e}}class Vr extends Qt{constructor(e){super({connection:e.connection}),e.connection.client.addEventListener("close",()=>{this.events.removeAllListeners()},{once:!0})}errorWith(e){if(e instanceof Error){const{client:r}=this.data.connection,s=new Event("error");Object.defineProperty(s,"cause",{enumerable:!0,configurable:!1,value:e}),r.socket.dispatchEvent(s)}}passthrough(){this.data.connection.server.connect()}}function Xr(t){return({frame:e,defaults:r})=>{const s=t();if(s!=null){if(typeof s=="function"){const n=e instanceof Ie?e.data.request:e instanceof Qt?new Request(e.data.connection.client.url,{headers:{connection:"upgrade",upgrade:"websocket"}}):null;return _(n!=null,'Failed to coerce a network frame to a legacy `onUnhandledRequest` strategy: unknown frame protocol "%s"',e.protocol),s(n,{warning:r.warn,error:r.error})}return Me(e,s)}}}function es(t){return{status:t.status,statusText:t.statusText,headers:Object.fromEntries(t.headers.entries())}}var ts={},rs=/(%?)(%([sdijo]))/g;function ss(t,e){switch(e){case"s":return t;case"d":case"i":return Number(t);case"j":return JSON.stringify(t);case"o":{if(typeof t=="string")return t;const r=JSON.stringify(t);return r==="{}"||r==="[]"||/^\[object .+?\]$/.test(r)?t:r}}}function ye(t,...e){if(e.length===0)return t;let r=0,s=t.replace(rs,(n,o,i,a)=>{const h=e[r],l=ss(h,a);return o?n:(r++,l)});return r<e.length&&(s+=` ${e.slice(r).join(" ")}`),s=s.replace(/%{2,2}/g,"%"),s}var ns=2;function os(t){if(!t.stack)return;const e=t.stack.split(`
`);e.splice(1,ns),t.stack=e.join(`
`)}var is=class extends Error{constructor(t,...e){super(t),this.message=t,this.name="Invariant Violation",this.message=ye(t,...e),os(this)}},x=(t,e,...r)=>{if(!t)throw new is(e,...r)};x.as=(t,e,r,...s)=>{if(!e){const n=s.length===0?r:ye(r,...s);let o;try{o=Reflect.construct(t,[n])}catch{o=t(n)}throw o}};function st(){if(typeof navigator<"u"&&navigator.product==="ReactNative")return!0;if(typeof process<"u"){const t=process.type;return t==="renderer"||t==="worker"?!1:!!(process.versions&&process.versions.node)}return!1}var as=Object.defineProperty,cs=(t,e)=>{for(var r in e)as(t,r,{get:e[r],enumerable:!0})},Ve={};cs(Ve,{blue:()=>ds,gray:()=>Xe,green:()=>hs,red:()=>us,yellow:()=>ls});function ls(t){return`\x1B[33m${t}\x1B[0m`}function ds(t){return`\x1B[34m${t}\x1B[0m`}function Xe(t){return`\x1B[90m${t}\x1B[0m`}function us(t){return`\x1B[31m${t}\x1B[0m`}function hs(t){return`\x1B[32m${t}\x1B[0m`}var Ne=st(),Zt=class{constructor(t){this.name=t,this.prefix=`[${this.name}]`;const e=gt("DEBUG"),r=gt("LOG_LEVEL");e==="1"||e==="true"||typeof e<"u"&&this.name.startsWith(e)?(this.debug=oe(r,"debug")?j:this.debug,this.info=oe(r,"info")?j:this.info,this.success=oe(r,"success")?j:this.success,this.warning=oe(r,"warning")?j:this.warning,this.error=oe(r,"error")?j:this.error):(this.info=j,this.success=j,this.warning=j,this.error=j,this.only=j)}prefix;extend(t){return new Zt(`${this.name}:${t}`)}debug(t,...e){this.logEntry({level:"debug",message:Xe(t),positionals:e,prefix:this.prefix,colors:{prefix:"gray"}})}info(t,...e){this.logEntry({level:"info",message:t,positionals:e,prefix:this.prefix,colors:{prefix:"blue"}});const r=new ps;return(s,...n)=>{r.measure(),this.logEntry({level:"info",message:`${s} ${Xe(`${r.deltaTime}ms`)}`,positionals:n,prefix:this.prefix,colors:{prefix:"blue"}})}}success(t,...e){this.logEntry({level:"info",message:t,positionals:e,prefix:`✔ ${this.prefix}`,colors:{timestamp:"green",prefix:"green"}})}warning(t,...e){this.logEntry({level:"warning",message:t,positionals:e,prefix:`⚠ ${this.prefix}`,colors:{timestamp:"yellow",prefix:"yellow"}})}error(t,...e){this.logEntry({level:"error",message:t,positionals:e,prefix:`✖ ${this.prefix}`,colors:{timestamp:"red",prefix:"red"}})}only(t){t()}createEntry(t,e){return{timestamp:new Date,level:t,message:e}}logEntry(t){const{level:e,message:r,prefix:s,colors:n,positionals:o=[]}=t,i=this.createEntry(e,r),a=n?.timestamp||"gray",h=n?.prefix||"gray",l={timestamp:Ve[a],prefix:Ve[h]};this.getWriter(e)([l.timestamp(this.formatTimestamp(i.timestamp))].concat(s!=null?l.prefix(s):[]).concat(yt(r)).join(" "),...o.map(yt))}formatTimestamp(t){return`${t.toLocaleTimeString("en-GB")}:${t.getMilliseconds()}`}getWriter(t){switch(t){case"debug":case"success":case"info":return fs;case"warning":return gs;case"error":return ys}}},ps=class{startTime;endTime;deltaTime;constructor(){this.startTime=performance.now()}measure(){this.endTime=performance.now();const t=this.endTime-this.startTime;this.deltaTime=t.toFixed(2)}},j=()=>{};function fs(t,...e){if(Ne){process.stdout.write(ye(t,...e)+`
`);return}console.log(t,...e)}function gs(t,...e){if(Ne){process.stderr.write(ye(t,...e)+`
`);return}console.warn(t,...e)}function ys(t,...e){if(Ne){process.stderr.write(ye(t,...e)+`
`);return}console.error(t,...e)}function gt(t){return Ne?ts[t]:globalThis[t]?.toString()}function oe(t,e){return t!==void 0&&t!==e}function yt(t){return typeof t>"u"?"undefined":t===null?"null":typeof t=="string"?t:typeof t=="object"?JSON.stringify(t):t.toString()}var ms=class extends Error{constructor(t,e,r){super(`Possible EventEmitter memory leak detected. ${r} ${e.toString()} listeners added. Use emitter.setMaxListeners() to increase limit`),this.emitter=t,this.type=e,this.count=r,this.name="MaxListenersExceededWarning"}},Yt=class{static listenerCount(t,e){return t.listenerCount(e)}constructor(){this.events=new Map,this.maxListeners=Yt.defaultMaxListeners,this.hasWarnedAboutPotentialMemoryLeak=!1}_emitInternalEvent(t,e,r){this.emit(t,e,r)}_getListeners(t){return Array.prototype.concat.apply([],this.events.get(t))||[]}_removeListener(t,e){const r=t.indexOf(e);return r>-1&&t.splice(r,1),[]}_wrapOnceListener(t,e){const r=(...s)=>(this.removeListener(t,r),e.apply(this,s));return Object.defineProperty(r,"name",{value:e.name}),r}setMaxListeners(t){return this.maxListeners=t,this}getMaxListeners(){return this.maxListeners}eventNames(){return Array.from(this.events.keys())}emit(t,...e){const r=this._getListeners(t);return r.forEach(s=>{s.apply(this,e)}),r.length>0}addListener(t,e){this._emitInternalEvent("newListener",t,e);const r=this._getListeners(t).concat(e);if(this.events.set(t,r),this.maxListeners>0&&this.listenerCount(t)>this.maxListeners&&!this.hasWarnedAboutPotentialMemoryLeak){this.hasWarnedAboutPotentialMemoryLeak=!0;const s=new ms(this,t,this.listenerCount(t));console.warn(s)}return this}on(t,e){return this.addListener(t,e)}once(t,e){return this.addListener(t,this._wrapOnceListener(t,e))}prependListener(t,e){const r=this._getListeners(t);if(r.length>0){const s=[e].concat(r);this.events.set(t,s)}else this.events.set(t,r.concat(e));return this}prependOnceListener(t,e){return this.prependListener(t,this._wrapOnceListener(t,e))}removeListener(t,e){const r=this._getListeners(t);return r.length>0&&(this._removeListener(r,e),this.events.set(t,r),this._emitInternalEvent("removeListener",t,e)),this}off(t,e){return this.removeListener(t,e)}removeAllListeners(t){return t?this.events.delete(t):this.events.clear(),this}listeners(t){return Array.from(this._getListeners(t))}listenerCount(t){return this._getListeners(t).length}rawListeners(t){return this.listeners(t)}},Jt=Yt;Jt.defaultMaxListeners=10;var bs="x-interceptors-internal-request-id";function mt(t){return globalThis[t]||void 0}function ws(t,e){globalThis[t]=e}function vs(t){delete globalThis[t]}var M=(function(t){return t.INACTIVE="INACTIVE",t.APPLYING="APPLYING",t.APPLIED="APPLIED",t.DISPOSING="DISPOSING",t.DISPOSED="DISPOSED",t})({}),nt=class{constructor(t){this.symbol=t,this.readyState=M.INACTIVE,this.emitter=new Jt,this.subscriptions=[],this.logger=new Zt(t.description),this.emitter.setMaxListeners(0),this.logger.info("constructing the interceptor...")}checkEnvironment(){return!0}apply(){const t=this.logger.extend("apply");if(t.info("applying the interceptor..."),this.readyState===M.APPLIED){t.info("intercepted already applied!");return}if(!this.checkEnvironment()){t.info("the interceptor cannot be applied in this environment!");return}this.readyState=M.APPLYING;const e=this.getInstance();if(e){t.info("found a running instance, reusing..."),this.on=(r,s)=>(t.info('proxying the "%s" listener',r),e.emitter.addListener(r,s),this.subscriptions.push(()=>{e.emitter.removeListener(r,s),t.info('removed proxied "%s" listener!',r)}),this),this.readyState=M.APPLIED;return}t.info("no running instance found, setting up a new instance..."),this.setup(),this.setInstance(),this.readyState=M.APPLIED}setup(){}on(t,e){const r=this.logger.extend("on");return this.readyState===M.DISPOSING||this.readyState===M.DISPOSED?(r.info("cannot listen to events, already disposed!"),this):(r.info('adding "%s" event listener:',t,e),this.emitter.on(t,e),this)}once(t,e){return this.emitter.once(t,e),this}off(t,e){return this.emitter.off(t,e),this}removeAllListeners(t){return this.emitter.removeAllListeners(t),this}dispose(){const t=this.logger.extend("dispose");if(this.readyState===M.DISPOSED){t.info("cannot dispose, already disposed!");return}if(t.info("disposing the interceptor..."),this.readyState=M.DISPOSING,!this.getInstance()){t.info("no interceptors running, skipping dispose...");return}if(this.clearInstance(),t.info("global symbol deleted:",mt(this.symbol)),this.subscriptions.length>0){t.info("disposing of %d subscriptions...",this.subscriptions.length);for(const e of this.subscriptions)e();this.subscriptions=[],t.info("disposed of all subscriptions!",this.subscriptions.length)}this.emitter.removeAllListeners(),t.info("destroyed the listener!"),this.readyState=M.DISPOSED}getInstance(){const t=mt(this.symbol);return this.logger.info("retrieved global instance:",t?.constructor?.name),t}setInstance(){ws(this.symbol,this),this.logger.info("set global instance!",this.symbol.description)}clearInstance(){vs(this.symbol),this.logger.info("cleared global instance!",this.symbol.description)}};function ot(){return Math.random().toString(16).slice(2)}function Kt(t){if(typeof t=="string")return Kt(new URL(t,typeof location<"u"?location.href:void 0));if(t.protocol==="http:"?t.protocol="ws:":t.protocol==="https:"&&(t.protocol="wss:"),t.protocol!=="ws:"&&t.protocol!=="wss:")throw new SyntaxError(`Failed to construct 'WebSocket': The URL's scheme must be either 'http', 'https', 'ws', or 'wss'. '${t.protocol}' is not allowed.`);if(t.hash!=="")throw new SyntaxError(`Failed to construct 'WebSocket': The URL contains a fragment identifier ('${t.hash}'). Fragment identifiers are not allowed in WebSocket URLs.`);return t.href}async function fe(t,e,...r){const s=t.listeners(e);if(s.length!==0)for(const n of s)await n.apply(t,r)}function it(t){const e=Object.getOwnPropertyDescriptor(globalThis,t);return typeof e>"u"||typeof e.get=="function"&&typeof e.get()>"u"||typeof e.get>"u"&&e.value==null?!1:typeof e.set>"u"&&!e.configurable?(console.error(`[MSW] Failed to apply interceptor: the global \`${t}\` property is non-configurable. This is likely an issue with your environment. If you are using a framework, please open an issue about this in their repository.`),!1):!0}function ks(){const t=(e,r)=>{t.state="pending",t.resolve=s=>{if(t.state!=="pending")return;t.result=s;const n=o=>(t.state="fulfilled",o);return e(s instanceof Promise?s:Promise.resolve(s).then(n))},t.reject=s=>{if(t.state==="pending")return queueMicrotask(()=>{t.state="rejected"}),r(t.rejectionReason=s)}};return t}var We=class extends Promise{#e;resolve;reject;constructor(t=null){const e=ks();super((r,s)=>{e(r,s),t?.(e.resolve,e.reject)}),this.#e=e,this.resolve=this.#e.resolve,this.reject=this.#e.reject}get state(){return this.#e.state}get rejectionReason(){return this.#e.rejectionReason}then(t,e){return this.#t(super.then(t,e))}catch(t){return this.#t(super.catch(t))}finally(t){return this.#t(super.finally(t))}#t(t){return Object.defineProperties(t,{resolve:{configurable:!0,value:this.resolve},reject:{configurable:!0,value:this.reject}})}};function S(t,e){return Object.defineProperties(e,{target:{value:t,enumerable:!0,writable:!0},currentTarget:{value:t,enumerable:!0,writable:!0}}),e}var te=Symbol("kCancelable"),T=Symbol("kDefaultPrevented"),at=class extends MessageEvent{constructor(t,e){super(t,e),this[te]=!!e.cancelable,this[T]=!1}get cancelable(){return this[te]}set cancelable(t){this[te]=t}get defaultPrevented(){return this[T]}set defaultPrevented(t){this[T]=t}preventDefault(){this.cancelable&&!this[T]&&(this[T]=!0)}},Ae=class extends Event{constructor(t,e={}){super(t,e),this.code=e.code===void 0?0:e.code,this.reason=e.reason===void 0?"":e.reason,this.wasClean=e.wasClean===void 0?!1:e.wasClean}},bt=class extends Ae{constructor(t,e={}){super(t,e),this[te]=!!e.cancelable,this[T]=!1}get cancelable(){return this[te]}set cancelable(t){this[te]=t}get defaultPrevented(){return this[T]}set defaultPrevented(t){this[T]=t}preventDefault(){this.cancelable&&!this[T]&&(this[T]=!0)}},ie=Symbol("kEmitter"),ke=Symbol("kBoundListener"),xs=class{constructor(t,e){this.socket=t,this.transport=e,this.id=ot(),this.url=new URL(t.url),this[ie]=new EventTarget,this.transport.addEventListener("outgoing",r=>{const s=S(this.socket,new at("message",{data:r.data,origin:r.origin,cancelable:!0}));this[ie].dispatchEvent(s),s.defaultPrevented&&r.preventDefault()}),this.transport.addEventListener("close",r=>{this[ie].dispatchEvent(S(this.socket,new Ae("close",r)))})}addEventListener(t,e,r){if(!Reflect.has(e,ke)){const s=e.bind(this.socket);Object.defineProperty(e,ke,{value:s,enumerable:!1,configurable:!1})}this[ie].addEventListener(t,Reflect.get(e,ke),r)}removeEventListener(t,e,r){this[ie].removeEventListener(t,Reflect.get(e,ke),r)}send(t){this.transport.send(t)}close(t,e){this.transport.close(t,e)}},wt="InvalidAccessError: close code out of user configurable range",Oe=Symbol("kPassthroughPromise"),Vt=Symbol("kOnSend"),ge=Symbol("kClose"),Ls=class extends EventTarget{static{this.CONNECTING=0}static{this.OPEN=1}static{this.CLOSING=2}static{this.CLOSED=3}constructor(t,e){super(),this.CONNECTING=0,this.OPEN=1,this.CLOSING=2,this.CLOSED=3,this._onopen=null,this._onmessage=null,this._onerror=null,this._onclose=null,this.url=Kt(t),this.protocol="",this.extensions="",this.binaryType="blob",this.readyState=this.CONNECTING,this.bufferedAmount=0,this[Oe]=new We,queueMicrotask(async()=>{await this[Oe]||(this.protocol=typeof e=="string"?e:Array.isArray(e)&&e.length>0?e[0]:"",this.readyState===this.CONNECTING&&(this.readyState=this.OPEN,this.dispatchEvent(S(this,new Event("open")))))})}set onopen(t){this.removeEventListener("open",this._onopen),this._onopen=t,t!==null&&this.addEventListener("open",t)}get onopen(){return this._onopen}set onmessage(t){this.removeEventListener("message",this._onmessage),this._onmessage=t,t!==null&&this.addEventListener("message",t)}get onmessage(){return this._onmessage}set onerror(t){this.removeEventListener("error",this._onerror),this._onerror=t,t!==null&&this.addEventListener("error",t)}get onerror(){return this._onerror}set onclose(t){this.removeEventListener("close",this._onclose),this._onclose=t,t!==null&&this.addEventListener("close",t)}get onclose(){return this._onclose}send(t){if(this.readyState===this.CONNECTING)throw this.close(),new DOMException("InvalidStateError");this.readyState===this.CLOSING||this.readyState===this.CLOSED||(this.bufferedAmount+=Ss(t),queueMicrotask(()=>{this.bufferedAmount=0,this[Vt]?.(t)}))}close(t=1e3,e){x(t,wt),x(t===1e3||t>=3e3&&t<=4999,wt),this[ge](t,e)}[ge](t=1e3,e,r=!0){this.readyState===this.CLOSING||this.readyState===this.CLOSED||(this.readyState=this.CLOSING,queueMicrotask(()=>{this.readyState=this.CLOSED,this.dispatchEvent(S(this,new Ae("close",{code:t,reason:e,wasClean:r}))),this._onopen=null,this._onmessage=null,this._onerror=null,this._onclose=null}))}addEventListener(t,e,r){return super.addEventListener(t,e,r)}removeEventListener(t,e,r){return super.removeEventListener(t,e,r)}};function Ss(t){return typeof t=="string"?t.length:t instanceof Blob?t.size:t.byteLength}var D=Symbol("kEmitter"),xe=Symbol("kBoundListener"),He=Symbol("kSend"),Es=class{constructor(t,e,r){this.client=t,this.transport=e,this.createConnection=r,this[D]=new EventTarget,this.mockCloseController=new AbortController,this.realCloseController=new AbortController,this.transport.addEventListener("outgoing",s=>{typeof this.realWebSocket>"u"||queueMicrotask(()=>{s.defaultPrevented||this[He](s.data)})}),this.transport.addEventListener("incoming",this.handleIncomingMessage.bind(this))}get socket(){return x(this.realWebSocket,'Cannot access "socket" on the original WebSocket server object: the connection is not open. Did you forget to call `server.connect()`?'),this.realWebSocket}connect(){x(!this.realWebSocket||this.realWebSocket.readyState!==WebSocket.OPEN,'Failed to call "connect()" on the original WebSocket instance: the connection already open');const t=this.createConnection();t.binaryType=this.client.binaryType,t.addEventListener("open",e=>{this[D].dispatchEvent(S(this.realWebSocket,new Event("open",e)))},{once:!0}),t.addEventListener("message",e=>{this.transport.dispatchEvent(S(this.realWebSocket,new MessageEvent("incoming",{data:e.data,origin:e.origin})))}),this.client.addEventListener("close",e=>{this.handleMockClose(e)},{signal:this.mockCloseController.signal}),t.addEventListener("close",e=>{this.handleRealClose(e)},{signal:this.realCloseController.signal}),t.addEventListener("error",()=>{const e=S(t,new Event("error",{cancelable:!0}));this[D].dispatchEvent(e),e.defaultPrevented||this.client.dispatchEvent(S(this.client,new Event("error")))}),this.realWebSocket=t}addEventListener(t,e,r){if(!Reflect.has(e,xe)){const s=e.bind(this.client);Object.defineProperty(e,xe,{value:s,enumerable:!1})}this[D].addEventListener(t,Reflect.get(e,xe),r)}removeEventListener(t,e,r){this[D].removeEventListener(t,Reflect.get(e,xe),r)}send(t){this[He](t)}[He](t){const{realWebSocket:e}=this;if(x(e,'Failed to call "server.send()" for "%s": the connection is not open. Did you forget to call "server.connect()"?',this.client.url),!(e.readyState===WebSocket.CLOSING||e.readyState===WebSocket.CLOSED)){if(e.readyState===WebSocket.CONNECTING){e.addEventListener("open",()=>{e.send(t)},{once:!0});return}e.send(t)}}close(){const{realWebSocket:t}=this;x(t,'Failed to close server connection for "%s": the connection is not open. Did you forget to call "server.connect()"?',this.client.url),this.realCloseController.abort(),!(t.readyState===WebSocket.CLOSING||t.readyState===WebSocket.CLOSED)&&(t.close(),queueMicrotask(()=>{this[D].dispatchEvent(S(this.realWebSocket,new bt("close",{code:1e3,cancelable:!0})))}))}handleIncomingMessage(t){const e=S(t.target,new at("message",{data:t.data,origin:t.origin,cancelable:!0}));this[D].dispatchEvent(e),e.defaultPrevented||this.client.dispatchEvent(S(this.client,new MessageEvent("message",{data:t.data,origin:t.origin})))}handleMockClose(t){this.realWebSocket&&this.realWebSocket.close()}handleRealClose(t){this.mockCloseController.abort();const e=S(this.realWebSocket,new bt("close",{code:t.code,reason:t.reason,wasClean:t.wasClean,cancelable:!0}));this[D].dispatchEvent(e),e.defaultPrevented||this.client[ge](t.code,t.reason)}},_s=class extends EventTarget{constructor(t){super(),this.socket=t,this.socket.addEventListener("close",e=>{this.dispatchEvent(S(this.socket,new Ae("close",e)))}),this.socket[Vt]=e=>{this.dispatchEvent(S(this.socket,new at("outgoing",{data:e,origin:this.socket.url,cancelable:!0})))}}addEventListener(t,e,r){return super.addEventListener(t,e,r)}dispatchEvent(t){return super.dispatchEvent(t)}send(t){queueMicrotask(()=>{if(this.socket.readyState===this.socket.CLOSING||this.socket.readyState===this.socket.CLOSED)return;const e=()=>{this.socket.dispatchEvent(S(this.socket,new MessageEvent("message",{data:t,origin:this.socket.url})))};this.socket.readyState===this.socket.CONNECTING?this.socket.addEventListener("open",()=>{e()},{once:!0}):e()})}close(t,e){this.socket[ge](t,e)}},qs=class Xt extends nt{static{this.symbol=Symbol("websocket")}constructor(){super(Xt.symbol)}checkEnvironment(){return it("WebSocket")}setup(){const e=Object.getOwnPropertyDescriptor(globalThis,"WebSocket"),r=new Proxy(globalThis.WebSocket,{construct:(s,n,o)=>{const[i,a]=n,h=()=>Reflect.construct(s,n,o),l=new Ls(i,a),m=new _s(l);return queueMicrotask(async()=>{try{const f=new Es(l,m,h),p=this.emitter.listenerCount("connection")>0;await fe(this.emitter,"connection",{client:new xs(l,m),server:f,info:{protocols:a}}),p?l[Oe].resolve(!1):(l[Oe].resolve(!0),f.connect(),f.addEventListener("open",()=>{l.dispatchEvent(S(l,new Event("open"))),f.realWebSocket&&(l.protocol=f.realWebSocket.protocol)}))}catch(f){f instanceof Error&&(l.dispatchEvent(new Event("error")),l.readyState!==WebSocket.CLOSING&&l.readyState!==WebSocket.CLOSED&&l[ge](1011,f.message,!1),console.error(f))}}),l}});Object.defineProperty(globalThis,"WebSocket",{value:r,configurable:!0}),this.subscriptions.push(()=>{Object.defineProperty(globalThis,"WebSocket",e)})}};function ct(){return typeof navigator<"u"&&"serviceWorker"in navigator&&typeof location<"u"&&location.protocol!=="file:"}function Cs(){try{const t=new ReadableStream({start:r=>r.close()});return new MessageChannel().port1.postMessage(t,[t]),!0}catch{return!1}}function Rs(){const t=((e,r)=>{t.state="pending",t.resolve=s=>{if(t.state!=="pending")return;t.result=s;const n=o=>(t.state="fulfilled",o);return e(s instanceof Promise?s:Promise.resolve(s).then(n))},t.reject=s=>{if(t.state==="pending")return queueMicrotask(()=>{t.state="rejected"}),r(t.rejectionReason=s)}});return t}var ae=class extends Promise{#e;resolve;reject;constructor(t=null){const e=Rs();super((r,s)=>{e(r,s),t?.(e.resolve,e.reject)}),this.#e=e,this.resolve=this.#e.resolve,this.reject=this.#e.reject}get state(){return this.#e.state}get rejectionReason(){return this.#e.rejectionReason}then(t,e){return this.#t(super.then(t,e))}catch(t){return this.#t(super.catch(t))}finally(t){return this.#t(super.finally(t))}#t(t){return Object.defineProperties(t,{resolve:{configurable:!0,value:this.resolve},reject:{configurable:!0,value:this.reject}})}},re=Symbol("isPatchedModule"),_e=class er extends Error{constructor(e){super(e),this.name="InterceptorError",Object.setPrototypeOf(this,er.prototype)}},ue=class H{static{this.PENDING=0}static{this.PASSTHROUGH=1}static{this.RESPONSE=2}static{this.ERROR=3}constructor(e,r){this.request=e,this.source=r,this.readyState=H.PENDING,this.handled=new We}get#e(){return this.handled}async passthrough(){x.as(_e,this.readyState===H.PENDING,'Failed to passthrough the "%s %s" request: the request has already been handled',this.request.method,this.request.url),this.readyState=H.PASSTHROUGH,await this.source.passthrough(),this.#e.resolve()}respondWith(e){x.as(_e,this.readyState===H.PENDING,'Failed to respond to the "%s %s" request with "%d %s": the request has already been handled (%d)',this.request.method,this.request.url,e.status,e.statusText||"OK",this.readyState),this.readyState=H.RESPONSE,this.#e.resolve(),this.source.respondWith(e)}errorWith(e){x.as(_e,this.readyState===H.PENDING,'Failed to error the "%s %s" request with "%s": the request has already been handled (%d)',this.request.method,this.request.url,e?.toString(),this.readyState),this.readyState=H.ERROR,this.source.errorWith(e),this.#e.resolve()}};function tr(t){try{return new URL(t),!0}catch{return!1}}function vt(t,e){const r=Object.getOwnPropertySymbols(e).find(s=>s.description===t);if(r)return Reflect.get(e,r)}var Q=class V extends Response{static{this.STATUS_CODES_WITHOUT_BODY=[101,103,204,205,304]}static{this.STATUS_CODES_WITH_REDIRECT=[301,302,303,307,308]}static isConfigurableStatusCode(e){return e>=200&&e<=599}static isRedirectResponse(e){return V.STATUS_CODES_WITH_REDIRECT.includes(e)}static isResponseWithBody(e){return!V.STATUS_CODES_WITHOUT_BODY.includes(e)}static setUrl(e,r){if(!e||e==="about:"||!tr(e))return;const s=vt("state",r);s?s.urlList.push(new URL(e)):Object.defineProperty(r,"url",{value:e,enumerable:!0,configurable:!0,writable:!1})}static parseRawHeaders(e){const r=new Headers;for(let s=0;s<e.length;s+=2)r.append(e[s],e[s+1]);return r}constructor(e,r={}){const s=r.status??200,n=V.isConfigurableStatusCode(s)?s:200,o=V.isResponseWithBody(s)?e:null;if(super(o,{status:n,statusText:r.statusText,headers:r.headers}),s!==n){const i=vt("state",this);i?i.status=s:Object.defineProperty(this,"status",{value:s,enumerable:!0,configurable:!0,writable:!1})}V.setUrl(r.url,this)}},js=Symbol("kRawRequest");function rr(t,e){Reflect.set(t,js,e)}var Ms=new TextEncoder;function Ps(t){return Ms.encode(t)}function Os(t,e){return new TextDecoder(e).decode(t)}function Is(t){return t.buffer.slice(t.byteOffset,t.byteOffset+t.byteLength)}async function Ts(t){try{return[null,await t().catch(e=>{throw e})]}catch(e){return[e,null]}}function Ns(t){return new URL(t,location.href).href}function Ue(t,e,r){return[t.active,t.installing,t.waiting].filter(i=>i!=null).find(i=>r(i.scriptURL,e))||null}var Ws=async(t,e={},r)=>{const s=Ns(t),n=await navigator.serviceWorker.getRegistrations().then(h=>h.filter(l=>Ue(l,s,r)));!navigator.serviceWorker.controller&&n.length>0&&location.reload();const[o]=n;if(o)return o.update(),[Ue(o,s,r),o];const[i,a]=await Ts(async()=>{const h=await navigator.serviceWorker.register(t,e);return[Ue(h,s,r),h]});if(i){if(i.message.includes("(404)")){const l=new URL(e?.scope||"/",location.href);throw new Error(v.formatMessage(`Failed to register a Service Worker for scope ('${l.href}') with script ('${s}'): Service Worker script does not exist at the given path.
...
Please consider using a custom "serviceWorker.url" option to point to the actual worker script location, or a custom "findWorker" option to resolve the Service Worker registration manually. More details: https://mswjs.io/docs/api/setup-worker/start`,e,location.host);throw new Error(n)}return this.workerPromise.state==="pending"?this.workerPromise.resolve([r,s]):this.workerPromise=new ae(n=>{n([r,s])}),this.#n.on("REQUEST",this.#p.bind(this)),this.#n.on("RESPONSE",this.#c.bind(this)),window.addEventListener("beforeunload",()=>{r.state!=="redundant"&&this.#n.postMessage("CLIENT_CLOSED"),clearInterval(this.#o),window.postMessage({type:"msw/worker:stop"})},{signal:this.#r?.signal}),await this.#d().catch(n=>{v.error("Error while checking the worker script integrity. Please report this on GitHub (https://github.com/mswjs/msw/issues) and include the original error below."),console.error(n)}),this.#o=window.setInterval(()=>{this.#n.postMessage("KEEPALIVE_REQUEST")},5e3),this.#t.quiet||$s(s),[r,s]}async#p(e){if(this.#i&&e.data.interceptedAt>this.#i)return e.postMessage("PASSTHROUGH");const r=Lt(e.data);mr.cache.set(r,r.clone());const s=new Fs({event:e,request:r});this.#s.set(e.data.id,s),await this.queue(s)}async#c(e){const{request:r,response:s,isMockedResponse:n}=e.data,o=this.#s.get(r.id);if(s.type?.includes("opaque")){this.#s.delete(r.id),o?.events.removeAllListeners();return}if(this.#s.delete(r.id),o==null)return;const i=Lt(r),a=s.status===0?Response.error():new Q(Q.isResponseWithBody(s.status)?s.body:null,{...s,url:r.url});try{o.events.emit(new Ut(n?"response:mocked":"response:bypass",{requestId:o.data.id,request:i,response:a,isMockedResponse:n}))}finally{o.events.removeAllListeners()}}#l=(e,r)=>e===r;async#d(){const e=new ae;return this.#n.postMessage("INTEGRITY_CHECK_REQUEST"),this.#n.once("INTEGRITY_CHECK_RESPONSE",r=>{const{checksum:s,packageVersion:n}=r.data;s!=="4db4a41e972cec1b64cc569c66952d82"&&v.warn(`The currently registered Service Worker has been generated by a different version of MSW (${n}) and may not be fully compatible with the installed version.

It's recommended you update your worker script by running this command:

  • npx msw init <PUBLIC_DIR>

You can also automate this process and make the worker script update automatically upon the library installations. Read more: https://mswjs.io/docs/cli/init.`),e.resolve()}),e}async#u(){if(this.workerPromise.state==="rejected")return;x(this.#a!=null,"[ServiceWorkerSource] Failed to print a start message: client confirmation not received");const e=await this.#a,[r,s]=await this.workerPromise;console.groupCollapsed(`%c${v.formatMessage("Mocking enabled.")}`,"color:orangered;font-weight:bold;"),console.log("%cDocumentation: %chttps://mswjs.io/docs","font-weight:bold","font-weight:normal"),console.log("Found an issue? https://github.com/mswjs/msw/issues"),console.log("Worker script URL:",r.scriptURL),console.log("Worker scope:",s.scope),e&&console.log("Client ID: %s (%s)",e.id,e.frameType),console.groupEnd()}#f(){console.log(`%c${v.formatMessage("Mocking disabled.")}`,"color:orangered;font-weight:bold;")}},Fs=class extends Ie{#e;constructor(t){super({request:t.request}),this.#e=t.event}passthrough(){this.#e.postMessage("PASSTHROUGH")}respondWith(t){t&&this.#t(t)}errorWith(t){if(t instanceof Response)return this.respondWith(t);v.warn(`Uncaught exception in the request handler for "%s %s". This exception has been gracefully handled as a 500 response, however, it's strongly recommended to resolve this error, as it indicates a mistake in your code. If you wish to mock an error response, please see this guide: https://mswjs.io/docs/http/mocking-responses/error-responses`,this.data.request.method,this.data.request.url);const e=t instanceof Error?t:new Error(t?.toString()||"Request failure");this.respondWith(c.json({name:e.name,message:e.message,stack:e.stack},{status:500,statusText:"Request Handler Error"}))}async#t(t){let e,r;const s=es(t);Cs()?(e=t.body,r=t.body==null?void 0:[t.body]):e=t.body==null?null:await t.clone().arrayBuffer(),this.#e.postMessage("MOCK_RESPONSE",{...s,body:e},r)}},sr=async t=>{try{return{error:null,data:await t().catch(r=>{throw r})}}catch(e){return{error:e,data:null}}};function nr(t,e=!1){return e?Object.prototype.toString.call(t).startsWith("[object "):Object.prototype.toString.call(t)==="[object Object]"}function qe(t,e){try{return t[e],!0}catch{return!1}}function Qs(t){return new Response(JSON.stringify(t instanceof Error?{name:t.name,message:t.message,stack:t.stack}:t),{status:500,statusText:"Unhandled Exception",headers:{"Content-Type":"application/json"}})}function lt(t){return t!=null&&t instanceof Response&&qe(t,"type")&&t.type==="error"}function zs(t){return nr(t,!0)&&qe(t,"status")&&qe(t,"statusText")&&qe(t,"bodyUsed")}function Zs(t){return t==null||!(t instanceof Error)?!1:"code"in t&&"errno"in t}async function or(t){const e=async o=>o instanceof Error?(await t.controller.errorWith(o),!0):lt(o)?(await t.controller.respondWith(o),!0):zs(o)?(await t.controller.respondWith(o),!0):nr(o)?(await t.controller.errorWith(o),!0):!1,r=async o=>{if(o instanceof _e)throw n.error;return Zs(o)?(await t.controller.errorWith(o),!0):o instanceof Response?await e(o):!1},s=new We;if(t.request.signal){if(t.request.signal.aborted){await t.controller.errorWith(t.request.signal.reason);return}t.request.signal.addEventListener("abort",()=>{s.reject(t.request.signal.reason)},{once:!0})}const n=await sr(async()=>{const o=fe(t.emitter,"request",{requestId:t.requestId,request:t.request,controller:t.controller});await Promise.race([s,o,t.controller.handled])});if(s.state==="rejected"){await t.controller.errorWith(s.rejectionReason);return}if(n.error){if(await r(n.error))return;if(t.emitter.listenerCount("unhandledException")>0){const o=new ue(t.request,{passthrough(){},async respondWith(i){await e(i)},async errorWith(i){await t.controller.errorWith(i)}});if(await fe(t.emitter,"unhandledException",{error:n.error,request:t.request,requestId:t.requestId,controller:o}),o.readyState!==ue.PENDING)return}await t.controller.respondWith(Qs(n.error));return}return t.controller.readyState===ue.PENDING?await t.controller.passthrough():t.controller.handled}function B(t){return Object.assign(new TypeError("Failed to fetch"),{cause:t})}var Ys=["content-encoding","content-language","content-location","content-type","content-length"],$e=Symbol("kRedirectCount");async function Js(t,e){if(e.status!==303&&t.body!=null)return Promise.reject(B());const r=new URL(t.url);let s;try{s=new URL(e.headers.get("location"),t.url)}catch(i){return Promise.reject(B(i))}if(!(s.protocol==="http:"||s.protocol==="https:"))return Promise.reject(B("URL scheme must be a HTTP(S) scheme"));if(Reflect.get(t,$e)>20)return Promise.reject(B("redirect count exceeded"));if(Object.defineProperty(t,$e,{value:(Reflect.get(t,$e)||0)+1}),t.mode==="cors"&&(s.username||s.password)&&!Et(r,s))return Promise.reject(B('cross origin not allowed for request mode "cors"'));const n={};([301,302].includes(e.status)&&t.method==="POST"||e.status===303&&!["HEAD","GET"].includes(t.method))&&(n.method="GET",n.body=null,Ys.forEach(i=>{t.headers.delete(i)})),Et(r,s)||(t.headers.delete("authorization"),t.headers.delete("proxy-authorization"),t.headers.delete("cookie"),t.headers.delete("host")),n.headers=t.headers;const o=await fetch(new Request(s,n));return Object.defineProperty(o,"redirected",{value:!0,configurable:!0}),o}function Et(t,e){return t.origin===e.origin&&t.origin==="null"||t.protocol===e.protocol&&t.hostname===e.hostname&&t.port===e.port}var Ks=class extends TransformStream{constructor(){console.warn("[Interceptors]: Brotli decompression of response streams is not supported in the browser"),super({transform(t,e){e.enqueue(t)}})}},Vs=class extends TransformStream{constructor(t,...e){super({},...e);const r=[super.readable,...t].reduce((s,n)=>s.pipeThrough(n));Object.defineProperty(this,"readable",{get(){return r}})}};function Xs(t){return t.toLowerCase().split(",").map(e=>e.trim())}function en(t){if(t==="")return null;const e=Xs(t);return e.length===0?null:new Vs(e.reduceRight((r,s)=>s==="gzip"||s==="x-gzip"?r.concat(new DecompressionStream("gzip")):s==="deflate"?r.concat(new DecompressionStream("deflate")):s==="br"?r.concat(new Ks):(r.length=0,r),[]))}function tn(t){if(t.body===null)return null;const e=en(t.headers.get("content-encoding")||"");return e?(t.body.pipeTo(e.writable),e.readable):null}var rn=class ir extends nt{static{this.symbol=Symbol("fetch")}constructor(){super(ir.symbol)}checkEnvironment(){return it("fetch")}async setup(){const e=globalThis.fetch;x(!e[re],'Failed to patch the "fetch" module: already patched.'),globalThis.fetch=async(r,s)=>{const n=ot(),o=typeof r=="string"&&typeof location<"u"&&!tr(r)?new URL(r,location.href):r,i=new Request(o,s);r instanceof Request&&rr(i,r);const a=new We,h=new ue(i,{passthrough:async()=>{this.logger.info("request has not been handled, passthrough...");const l=i.clone(),{error:m,data:f}=await sr(()=>e(i));if(m)return a.reject(m);if(this.logger.info("original fetch performed",f),this.emitter.listenerCount("response")>0){this.logger.info('emitting the "response" event...');const p=f.clone();await fe(this.emitter,"response",{response:p,isMockedResponse:!1,request:l,requestId:n})}a.resolve(f)},respondWith:async l=>{if(lt(l)){this.logger.info("request has errored!",{response:l}),a.reject(B(l));return}this.logger.info("received mocked response!",{rawResponse:l});const m=tn(l),f=m===null?l:new Q(m,l);if(Q.setUrl(i.url,f),Q.isRedirectResponse(f.status)){if(i.redirect==="error"){a.reject(B("unexpected redirect"));return}if(i.redirect==="follow"){Js(i,f).then(p=>{a.resolve(p)},p=>{a.reject(p)});return}}this.emitter.listenerCount("response")>0&&(this.logger.info('emitting the "response" event...'),await fe(this.emitter,"response",{response:f.clone(),isMockedResponse:!0,request:i,requestId:n})),a.resolve(f)},errorWith:l=>{this.logger.info("request has been aborted!",{reason:l}),a.reject(l)}});return this.logger.info("[%s] %s",i.method,i.url),this.logger.info("awaiting for the mocked response..."),this.logger.info('emitting the "request" event for %s listener(s)...',this.emitter.listenerCount("request")),await or({request:i,requestId:n,emitter:this.emitter,controller:h}),a},Object.defineProperty(globalThis.fetch,re,{enumerable:!0,configurable:!0,value:!0}),this.subscriptions.push(()=>{Object.defineProperty(globalThis.fetch,re,{value:void 0}),globalThis.fetch=e,this.logger.info('restored native "globalThis.fetch"!',globalThis.fetch.name)})}};function sn(t,e){const r=new Uint8Array(t.byteLength+e.byteLength);return r.set(t,0),r.set(e,t.byteLength),r}var ar=class{constructor(t,e){this.NONE=0,this.CAPTURING_PHASE=1,this.AT_TARGET=2,this.BUBBLING_PHASE=3,this.type="",this.srcElement=null,this.currentTarget=null,this.eventPhase=0,this.isTrusted=!0,this.composed=!1,this.cancelable=!0,this.defaultPrevented=!1,this.bubbles=!0,this.lengthComputable=!0,this.loaded=0,this.total=0,this.cancelBubble=!1,this.returnValue=!0,this.type=t,this.target=e?.target||null,this.currentTarget=e?.currentTarget||null,this.timeStamp=Date.now()}composedPath(){return[]}initEvent(t,e,r){this.type=t,this.bubbles=!!e,this.cancelable=!!r}preventDefault(){this.defaultPrevented=!0}stopPropagation(){}stopImmediatePropagation(){}},nn=class extends ar{constructor(t,e){super(t),this.lengthComputable=e?.lengthComputable||!1,this.composed=e?.composed||!1,this.loaded=e?.loaded||0,this.total=e?.total||0}},on=typeof ProgressEvent<"u";function an(t,e,r){const s=["error","progress","loadstart","loadend","load","timeout","abort"],n=on?ProgressEvent:nn;return s.includes(e)?new n(e,{lengthComputable:!0,loaded:r?.loaded||0,total:r?.total||0}):new ar(e,{target:t,currentTarget:t})}function cr(t,e){if(!(e in t))return null;if(Object.prototype.hasOwnProperty.call(t,e))return t;const r=Reflect.getPrototypeOf(t);return r?cr(r,e):null}function Be(t,e){return new Proxy(t,cn(e))}function cn(t){const{constructorCall:e,methodCall:r,getProperty:s,setProperty:n}=t,o={};return typeof e<"u"&&(o.construct=function(i,a,h){const l=Reflect.construct.bind(null,i,a,h);return e.call(h,a,l)}),o.set=function(i,a,h){const l=()=>{const m=cr(i,a)||i,f=Reflect.getOwnPropertyDescriptor(m,a);return typeof f?.set<"u"?(f.set.apply(i,[h]),!0):Reflect.defineProperty(m,a,{writable:!0,enumerable:!0,configurable:!0,value:h})};return typeof n<"u"?n.call(i,[a,h],l):l()},o.get=function(i,a,h){const l=()=>i[a],m=typeof s<"u"?s.call(i,[a,h],l):l();return typeof m=="function"?(...f)=>{const p=m.bind(i,...f);return typeof r<"u"?r.call(i,[a,f],p):p()}:m},o}function ln(t){return["application/xhtml+xml","application/xml","image/svg+xml","text/html","text/xml"].some(e=>t.startsWith(e))}function dn(t){try{return JSON.parse(t)}catch{return null}}function un(t,e){return new Q(Q.isResponseWithBody(t.status)?e:null,{url:t.responseURL,status:t.status,statusText:t.statusText,headers:hn(t.getAllResponseHeaders())})}function hn(t){const e=new Headers,r=t.split(/[\r\n]+/);for(const s of r){if(s.trim()==="")continue;const[n,...o]=s.split(": "),i=o.join(": ");e.append(n,i)}return e}async function _t(t){const e=t.headers.get("content-length");return e!=null&&e!==""?Number(e):(await t.arrayBuffer()).byteLength}var ce=Symbol("kIsRequestHandled"),pn=st(),Fe=Symbol("kFetchRequest"),fn=class{constructor(t,e){this.initialRequest=t,this.logger=e,this.method="GET",this.url=null,this[ce]=!1,this.events=new Map,this.uploadEvents=new Map,this.requestId=ot(),this.requestHeaders=new Headers,this.responseBuffer=new Uint8Array,this.request=Be(t,{setProperty:([r,s],n)=>{switch(r){case"ontimeout":{const o=r.slice(2);return this.request.addEventListener(o,s),n()}default:return n()}},methodCall:([r,s],n)=>{switch(r){case"open":{const[o,i]=s;return typeof i>"u"?(this.method="GET",this.url=qt(o)):(this.method=o,this.url=qt(i)),this.logger=this.logger.extend(`${this.method} ${this.url.href}`),this.logger.info("open",this.method,this.url.href),n()}case"addEventListener":{const[o,i]=s;return this.registerEvent(o,i),this.logger.info("addEventListener",o,i),n()}case"setRequestHeader":{const[o,i]=s;return this.requestHeaders.set(o,i),this.logger.info("setRequestHeader",o,i),n()}case"send":{const[o]=s;this.request.addEventListener("load",()=>{if(typeof this.onResponse<"u"){const h=un(this.request,this.request.response);this.onResponse.call(this,{response:h,isMockedResponse:this[ce],request:a,requestId:this.requestId})}});const i=typeof o=="string"?Ps(o):o,a=this.toFetchApiRequest(i);this[Fe]=a.clone(),queueMicrotask(()=>{(this.onRequest?.call(this,{request:a,requestId:this.requestId})||Promise.resolve()).finally(()=>{if(!this[ce])return this.logger.info("request callback settled but request has not been handled (readystate %d), performing as-is...",this.request.readyState),pn&&this.request.setRequestHeader(bs,this.requestId),n()})});break}default:return n()}}}),Z(this.request,"upload",Be(this.request.upload,{setProperty:([r,s],n)=>{switch(r){case"onloadstart":case"onprogress":case"onaboart":case"onerror":case"onload":case"ontimeout":case"onloadend":{const o=r.slice(2);this.registerUploadEvent(o,s)}}return n()},methodCall:([r,s],n)=>{switch(r){case"addEventListener":{const[o,i]=s;return this.registerUploadEvent(o,i),this.logger.info("upload.addEventListener",o,i),n()}}}}))}registerEvent(t,e){const r=(this.events.get(t)||[]).concat(e);this.events.set(t,r),this.logger.info('registered event "%s"',t,e)}registerUploadEvent(t,e){const r=(this.uploadEvents.get(t)||[]).concat(e);this.uploadEvents.set(t,r),this.logger.info('registered upload event "%s"',t,e)}async respondWith(t){if(this[ce]=!0,this[Fe]){const s=await _t(this[Fe]);this.trigger("loadstart",this.request.upload,{loaded:0,total:s}),this.trigger("progress",this.request.upload,{loaded:s,total:s}),this.trigger("load",this.request.upload,{loaded:s,total:s}),this.trigger("loadend",this.request.upload,{loaded:s,total:s})}this.logger.info("responding with a mocked response: %d %s",t.status,t.statusText),Z(this.request,"status",t.status),Z(this.request,"statusText",t.statusText),Z(this.request,"responseURL",this.url.href),this.request.getResponseHeader=new Proxy(this.request.getResponseHeader,{apply:(s,n,o)=>{if(this.logger.info("getResponseHeader",o[0]),this.request.readyState<this.request.HEADERS_RECEIVED)return this.logger.info("headers not received yet, returning null"),null;const i=t.headers.get(o[0]);return this.logger.info('resolved response header "%s" to',o[0],i),i}}),this.request.getAllResponseHeaders=new Proxy(this.request.getAllResponseHeaders,{apply:()=>{if(this.logger.info("getAllResponseHeaders"),this.request.readyState<this.request.HEADERS_RECEIVED)return this.logger.info("headers not received yet, returning empty string"),"";const s=Array.from(t.headers.entries()).map(([n,o])=>`${n}: ${o}`).join(`\r
`);return this.logger.info("resolved all response headers to",s),s}}),Object.defineProperties(this.request,{response:{enumerable:!0,configurable:!1,get:()=>this.response},responseText:{enumerable:!0,configurable:!1,get:()=>this.responseText},responseXML:{enumerable:!0,configurable:!1,get:()=>this.responseXML}});const e=await _t(t.clone());this.logger.info("calculated response body length",e),this.trigger("loadstart",this.request,{loaded:0,total:e}),this.setReadyState(this.request.HEADERS_RECEIVED),this.setReadyState(this.request.LOADING);const r=()=>{this.logger.info("finalizing the mocked response..."),this.setReadyState(this.request.DONE),this.trigger("load",this.request,{loaded:this.responseBuffer.byteLength,total:e}),this.trigger("loadend",this.request,{loaded:this.responseBuffer.byteLength,total:e})};if(t.body){this.logger.info("mocked response has body, streaming...");const s=t.body.getReader(),n=async()=>{const{value:o,done:i}=await s.read();if(i){this.logger.info("response body stream done!"),r();return}o&&(this.logger.info("read response body chunk:",o),this.responseBuffer=sn(this.responseBuffer,o),this.trigger("progress",this.request,{loaded:this.responseBuffer.byteLength,total:e})),n()};n()}else r()}responseBufferToText(){return Os(this.responseBuffer)}get response(){if(this.logger.info("getResponse (responseType: %s)",this.request.responseType),this.request.readyState!==this.request.DONE)return null;switch(this.request.responseType){case"json":{const t=dn(this.responseBufferToText());return this.logger.info("resolved response JSON",t),t}case"arraybuffer":{const t=Is(this.responseBuffer);return this.logger.info("resolved response ArrayBuffer",t),t}case"blob":{const t=this.request.getResponseHeader("Content-Type")||"text/plain",e=new Blob([this.responseBufferToText()],{type:t});return this.logger.info("resolved response Blob (mime type: %s)",e,t),e}default:{const t=this.responseBufferToText();return this.logger.info('resolving "%s" response type as text',this.request.responseType,t),t}}}get responseText(){if(x(this.request.responseType===""||this.request.responseType==="text","InvalidStateError: The object is in invalid state."),this.request.readyState!==this.request.LOADING&&this.request.readyState!==this.request.DONE)return"";const t=this.responseBufferToText();return this.logger.info('getResponseText: "%s"',t),t}get responseXML(){if(x(this.request.responseType===""||this.request.responseType==="document","InvalidStateError: The object is in invalid state."),this.request.readyState!==this.request.DONE)return null;const t=this.request.getResponseHeader("Content-Type")||"";return typeof DOMParser>"u"?(console.warn("Cannot retrieve XMLHttpRequest response body as XML: DOMParser is not defined. You are likely using an environment that is not browser or does not polyfill browser globals correctly."),null):ln(t)?new DOMParser().parseFromString(this.responseBufferToText(),t):null}errorWith(t){this[ce]=!0,this.logger.info("responding with an error"),this.setReadyState(this.request.DONE),this.trigger("error",this.request),this.trigger("loadend",this.request)}setReadyState(t){if(this.logger.info("setReadyState: %d -> %d",this.request.readyState,t),this.request.readyState===t){this.logger.info("ready state identical, skipping transition...");return}Z(this.request,"readyState",t),this.logger.info("set readyState to: %d",t),t!==this.request.UNSENT&&(this.logger.info('triggering "readystatechange" event...'),this.trigger("readystatechange",this.request))}trigger(t,e,r){const s=e[`on${t}`],n=an(e,t,r);this.logger.info('trigger "%s"',t,r||""),typeof s=="function"&&(this.logger.info('found a direct "%s" callback, calling...',t),s.call(e,n));const o=e instanceof XMLHttpRequestUpload?this.uploadEvents:this.events;for(const[i,a]of o)i===t&&(this.logger.info('found %d listener(s) for "%s" event, calling...',a.length,t),a.forEach(h=>h.call(e,n)))}toFetchApiRequest(t){this.logger.info("converting request to a Fetch API Request...");const e=t instanceof Document?t.documentElement.innerText:t,r=new Request(this.url.href,{method:this.method,headers:this.requestHeaders,credentials:this.request.withCredentials?"include":"same-origin",body:["GET","HEAD"].includes(this.method.toUpperCase())?null:e});return Z(r,"headers",Be(r.headers,{methodCall:([s,n],o)=>{switch(s){case"append":case"set":{const[i,a]=n;this.request.setRequestHeader(i,a);break}case"delete":{const[i]=n;console.warn(`XMLHttpRequest: Cannot remove a "${i}" header from the Fetch API representation of the "${r.method} ${r.url}" request. XMLHttpRequest headers cannot be removed.`);break}}return o()}})),rr(r,this.request),this.logger.info("converted request to a Fetch API Request!",r),r}};function qt(t){return typeof location>"u"?new URL(t):new URL(t.toString(),location.href)}function Z(t,e,r){Reflect.defineProperty(t,e,{writable:!0,enumerable:!0,value:r})}function gn({emitter:t,logger:e}){return new Proxy(globalThis.XMLHttpRequest,{construct(r,s,n){e.info("constructed new XMLHttpRequest");const o=Reflect.construct(r,s,n),i=Object.getOwnPropertyDescriptors(r.prototype);for(const h in i)Reflect.defineProperty(o,h,i[h]);const a=new fn(o,e);return a.onRequest=async function({request:h,requestId:l}){const m=new ue(h,{passthrough:()=>{this.logger.info("no mocked response received, performing request as-is...")},respondWith:async f=>{if(lt(f)){this.errorWith(new TypeError("Network error"));return}await this.respondWith(f)},errorWith:f=>{this.logger.info("request errored!",{error:f}),f instanceof Error&&this.errorWith(f)}});this.logger.info("awaiting mocked response..."),this.logger.info('emitting the "request" event for %s listener(s)...',t.listenerCount("request")),await or({request:h,requestId:l,controller:m,emitter:t})},a.onResponse=async function({response:h,isMockedResponse:l,request:m,requestId:f}){this.logger.info('emitting the "response" event for %s listener(s)...',t.listenerCount("response")),t.emit("response",{response:h,isMockedResponse:l,request:m,requestId:f})},a.request}})}var yn=class lr extends nt{static{this.interceptorSymbol=Symbol("xhr")}constructor(){super(lr.interceptorSymbol)}checkEnvironment(){return it("XMLHttpRequest")}setup(){const e=this.logger.extend("setup");e.info('patching "XMLHttpRequest" module...');const r=globalThis.XMLHttpRequest;x(!r[re],'Failed to patch the "XMLHttpRequest" module: already patched.'),globalThis.XMLHttpRequest=gn({emitter:this.emitter,logger:this.logger}),e.info('native "XMLHttpRequest" module patched!',globalThis.XMLHttpRequest.name),Object.defineProperty(globalThis.XMLHttpRequest,re,{enumerable:!0,configurable:!0,value:!0}),this.subscriptions.push(()=>{Object.defineProperty(globalThis.XMLHttpRequest,re,{value:void 0}),globalThis.XMLHttpRequest=r,e.info('native "XMLHttpRequest" module restored!',globalThis.XMLHttpRequest.name)})}},mn=class extends zt{constructor(t){super({interceptors:[new yn,new rn]}),this.options=t}enable(){super.enable(),this.options.quiet||this.#e()}disable(){super.disable(),this.options.quiet||this.#t()}#e(){console.groupCollapsed(`%c${v.formatMessage("Mocking enabled (fallback mode).")}`,"color:orangered;font-weight:bold;"),console.log("%cDocumentation: %chttps://mswjs.io/docs","font-weight:bold","font-weight:normal"),console.log("Found an issue? https://github.com/mswjs/msw/issues"),console.groupEnd()}#t(){console.log(`%c${v.formatMessage("Mocking disabled.")}`,"color:orangered;font-weight:bold;")}},bn="/mockServiceWorker.js";function wn(...t){x(!st(),v.formatMessage("Failed to execute `setupWorker` in a non-browser environment"));const e=Dr({sources:[],handlers:t});return{async start(r){if(r?.waitUntilReady!=null&&v.warn('The "waitUntilReady" option has been deprecated. Please remove it from this "worker.start()" call. Follow the recommended Browser integration (https://mswjs.io/docs/integrations/browser) to eliminate any race conditions between the Service Worker registration and any requests made by your application on initial render.'),e.readyState===Ke.ENABLED){v.warn('Found a redundant "worker.start()" call. Note that starting the worker while mocking is already enabled will have no effect. Consider removing this "worker.start()" call.');return}const s=ct()?await St.from({serviceWorker:{url:r?.serviceWorker?.url?.toString()||bn,options:r?.serviceWorker?.options},findWorker:r?.findWorker,quiet:r?.quiet}):new mn({quiet:r?.quiet});if(e.configure({sources:[s,new zt({interceptors:[new qs]})],onUnhandledFrame:Xr(()=>r?.onUnhandledRequest||"warn"),context:{quiet:r?.quiet}}),await e.enable(),s instanceof St){const[,n]=await s.workerPromise;return n}},stop(){if(e.readyState===Ke.DISABLED){v.warn('Found a redundant "worker.stop()" call. Notice that stopping the worker after it has already been stopped has no effect. Consider removing this "worker.stop()" call.');return}e.disable(),window.postMessage({type:"msw/worker:stop"})},events:e.events,use:e.use.bind(e),resetHandlers:e.resetHandlers.bind(e),restoreHandlers:e.restoreHandlers.bind(e),listHandlers:e.listHandlers.bind(e)}}const vn="Jeff Mercer",Ct={"mana.md":`# Mana — the builder
...
`},Rt=[{name:"profile.md",relPath:"00_Core/profile.md",group:"Profile",content:`# Jeff — profile
...
`,Ze="data:image/svg+xml,%3csvg%20xmlns='http://www.w3.org/2000/svg'%20viewBox='0%200%2036%2036'%3e%3cpath%20fill='%23C1694F'%20d='M31.595%2015.007c-1.75-3.623-5.934-9.053-9.531-9.053-4.071%200-8.228%207.259-10.071%2010.378-.176.299-.321.578-.464.857-3.371-1.182-.536-6.631-.536-10.463%200-.957-.138-1.637-.44-2.119.489-.606.586-1.347.192-1.699-.413-.367-1.195-.163-1.745.456-.08.089-.129.186-.189.28-.424-.067-.903-.102-1.472-.102-.565%200-2.916.266-4.229.791C-.007%205.582.993%209%201.993%209h4c1%200%20.756%202.31%200%204.726-.83%202.654-1.439%205.145-1%206.606.808%202.687%203.712%203.589%206.164%203.86%201.059%202.659%201.517%206.455%201.473%207.962-.059%202%201.94%202.059%201.999.059.036-1.211-.102-3.68.143-5.781.658%202.833.935%206.097.899%207.314-.059%201.998%201.94%202.057%201.999.059.047-1.602-.182-6.36.559-8.982.507.017%201.044.03%201.619.035%201.774.09%203.726.085%205.506-.015%201.05%201.592%201.996%202.991%201.982%203.435-.029%201-1.117%203.969-1.146%204.969-.029%201%20.94%202.029%201.999.059.648-1.205%201.324-3.419%201.536-5.421.171.364.274.656.269.843-.029%201-.97%203.93-.999%204.93-.029.998.941%202.027%201.999.059%201.059-1.971%201.998-4.898%201.058-6.928-.797-1.72.431-4.165.824-7.914%201.082%201.665%201.117%203.351%201.118%203.459%200%20.553.447%201%201%201%20.553%200%201-.447%201-1-.001-.215-.067-4.85-4.399-7.327z'/%3e%3cpath%20fill='%23292F33'%20d='M8.28%205.571c-.016.552-.477.986-1.029.97-.552-.016-.986-.477-.97-1.029.016-.552.477-.986%201.029-.97.552.016.986.477.97%201.029z'/%3e%3c/svg%3e",Tt="/assets/aurora-fjord-img-DAHDArrG.webp",On=`<div>
  <div class="rounded-lg overflow-hidden border border-line">
    <svg viewBox="0 0 1000 620" xmlns="http://www.w3.org/2000/svg" style="display:block;width:100%;height:auto;">
      ...
      <image href="${Ze}" x="335" y="488" width="90" height="70" filter="url(#dn-sil)" transform="matrix(0.478,0,0,0.478,718.439,153.876)"></image>
      <image href="${Ze}" x="475" y="502" width="90" height="70" filter="url(#dn-sil)" transform="matrix(0.632,0,0,0.632,518.958,59.096)"></image>
      <image href="${Ze}" x="615" y="492" width="90" height="70" filter="url(#dn-sil)" transform="matrix(0.738,0,0,0.738,289.2,-1.182)"></image>
    </svg>
  </div>
  ...
  <div class="rounded-lg overflow-hidden border border-line">
    ...
    <g data-obj="image-2" transform="matrix(0.476,0,0,0.476,583.156,-35.713)"><image href="${Tt}" xlink:href="${Tt}" x="396.7" y="155" width="206.59" height="310" preserveAspectRatio="xMidYMid meet"></image></g></svg>
  </div>
...
</div>`,dr=t=>`<!doctype html><html><head><meta charset="utf-8">
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .note { margin-top: 28px; padding: 14px 16px; border-radius: 12px; background: color-mix(in srgb, currentColor 7%, transparent); font-size: 15px; }
</style></head><body><div class="wrap">${t}</div></body></html>`,Tn=dr(`<p class="eyebrow">Mana, in one idea</p>
   ...
   <div class="note">This is a live demo. Everything works by hand on your own browser storage. The one thing it can't do here is think, that's what you unlock by running your own Mana with a model connected.</div>`),Nn=dr(`<p class="eyebrow">What's inside</p>
