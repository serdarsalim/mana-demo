...
Read more: https://mswjs.io/docs/http/intercepting-requests`}async resolve(e,r,s){const{id:n,request:o}=this.data,i=s?.quiet?null:o.clone();if(this.events.emit(new F("request:start",{requestId:n,request:o})),ds(o))return this.events.emit(new F("request:end",{requestId:n,request:o})),this.passthrough(),null;const[a,u]=await os(()=>ns({requestId:n,request:o,handlers:e,resolutionContext:{baseUrl:s?.baseUrl?.toString(),quiet:s?.quiet}}));if(a!=null)return this.events.emit(new ps("unhandledException",{error:a,requestId:n,request:o}))||(console.error(a),k.error('Encountered an unhandled exception during the handler lookup for "%s %s". Please see the original error above.',o.method,o.url)),this.errorWith(a),null;if(u==null)return this.events.emit(new F("request:unhandled",{requestId:n,request:o})),await Ye(this,r).then(()=>this.passthrough(),y=>this.errorWith(y)),this.events.emit(new F("request:end",{requestId:n,request:o})),!1;const{response:d,handler:m,parsedResult:p}=u;if(this.events.emit(new F("request:match",{requestId:n,request:o})),d==null)return this.events.emit(new F("request:end",{requestId:n,request:o})),this.passthrough(),null;if(us(d))return this.events.emit(new F("request:end",{requestId:n,request:o})),this.passthrough(),null;const f=s?.quiet?null:d.clone();return await as(o,d),this.respondWith(d),this.events.emit(new F("request:end",{requestId:n,request:o})),s?.quiet||m.log({request:i,response:f,parsedResult:p}),!0}}async function Ye(t,e){const r=async n=>{if(n==="bypass")return;const o=await t.getUnhandledMessage();switch(n){case"warn":return k.warn("Warning: %s",o);case"error":return k.error("Error: %s",o)}},s=async n=>{if(q.as(mt,n==="bypass"||n==="warn"||n==="error",k.formatMessage('Failed to react to an unhandled network frame: unknown strategy "%s". Please provide one of the supported strategies ("bypass", "warn", "error") or a custom callback function as the value of the "onUnhandledRequest" option.',n)),n!=="bypass"&&(await r(n),n==="error"))return Promise.reject(new mt(k.formatMessage('Cannot bypass a request when using the "error" strategy for the "onUnhandledRequest" option.')))};if(typeof e=="function")return e({frame:t,defaults:{warn:r.bind(null,"warn"),error:r.bind(null,"error")}});if(!(t instanceof Xe&&is(t.data.request)))return s(e)}function Tt(t){const e=[];for(const r of t)r instanceof Promise&&e.push(r);if(e.length>0)return Promise.all(e).then(()=>{})}var yt=(t=>(t[t.DISABLED=0]="DISABLED",t[t.ENABLED=1]="ENABLED",t))(yt||{});function fs(t){let e=0;const r=new vt,s=new rs,n=a=>a instanceof cr?a:new ts(a||[]);let o={...t},i=n(o.handlers);return{get readyState(){return e},events:r,configure(a){q(e===0,'Failed to call "configure()" on the network: cannot configure an already enabled network.'),a.handlers&&!Object.is(a.handlers,o.handlers)&&(i=n(a.handlers)),o={...o,...a}},enable(){q(e===0,'Failed to call "enable" on the network: already enabled'),e=1;const a={active:!0};s.subscriptions.push(()=>{a.active=!1});const u=o.sources.map(d=>(xt.prototype.disable.call(d),d.on("frame",async({frame:m})=>{m.events.on("*",f=>{a.active&&r.emit(f)});const p=m.getHandlers(i);await m.resolve(p,o.onUnhandledFrame||"warn",o.context)}),d.enable()));return Tt(u)},disable(){return q(e===1,'Failed to call "disable" on the network: already disabled'),e=0,s.dispose(),Tt(o.sources.map(a=>a.disable()))},use(...a){i.use(a)},resetHandlers(...a){i.reset(a)},restoreHandlers(){i.restore()},listHandlers(){return ss(i.currentHandlers())}}}async function gs(t,e,...r){const s=t.listeners(e);if(s.length!==0)for(const n of s)await n.apply(t,r)}var ms=class{#e=new Map;applyPatch(t,e,r){const s=this.#e.get(t);q(!s?.has(e),`Failed to replace a global value at "${String(e)}": already replaced.`);const n=hr(t,e);if(typeof n>"u")return console.warn(`Failed to replace a global value at "${String(e)}": not a global value.`),()=>{};if(n.descriptor.configurable)Object.defineProperty(t,e,{value:r(t[e]),enumerable:!0,configurable:!0});else if(n.descriptor.writable)t[e]=r(t[e]);else throw new Error(`Failed to patch a non-configurable non-writable property "${e.toString()}"`);const o=()=>{const i=this.#e.get(t);i?.has(e)&&(n.owner===t?Object.defineProperty(n.owner,e,n.descriptor):Reflect.deleteProperty(t,e),i.delete(e),i.size===0&&this.#e.delete(t))};return s?s.set(e,o):this.#e.set(t,new Map([[e,o]])),o}restoreAllPatches(){const t=[];for(const[,e]of this.#e)for(const[,r]of e)try{r()}catch(s){if(s instanceof Error)t.push(s);else throw s}if(t.length>0)throw new AggregateError(t,"FOO!")}};const ys=new ms;function hr(t,e){let r=t,s;for(;r;){if(s=Object.getOwnPropertyDescriptor(r,e),s)return{owner:r,descriptor:s};r=Object.getPrototypeOf(r)}}function ws(t){const e=hr(globalThis,t);if(typeof e>"u")return!1;const{descriptor:r}=e;return typeof r.get=="function"&&typeof r.get()>"u"||typeof r.get>"u"&&r.value==null?!1:typeof r.set>"u"&&!r.configurable?(console.error(`[MSW] Failed to apply interceptor: the global \`${t}\` property is non-configurable. This is likely an issue with your environment. If you are using a framework, please open an issue about this in their repository.`),!1):!0}function L(t,e){return Object.defineProperties(e,{target:{value:t,enumerable:!0,writable:!0},currentTarget:{value:t,enumerable:!0,writable:!0}}),e}const ue=Symbol("kCancelable"),H=Symbol("kDefaultPrevented");var St=class extends MessageEvent{constructor(e,r){super(e,r),this[ue]=!!r.cancelable,this[H]=!1}get cancelable(){return this[ue]}set cancelable(e){this[ue]=e}get defaultPrevented(){return this[H]}set defaultPrevented(e){this[H]=e}preventDefault(){this.cancelable&&!this[H]&&(this[H]=!0)}},Ve=class extends Event{constructor(e,r={}){super(e,r),this.code=r.code===void 0?0:r.code,this.reason=r.reason===void 0?"":r.reason,this.wasClean=r.wasClean===void 0?!1:r.wasClean}},Ot=class extends Ve{constructor(e,r={}){super(e,r),this[ue]=!!r.cancelable,this[H]=!1}get cancelable(){return this[ue]}set cancelable(e){this[ue]=e}get defaultPrevented(){return this[H]}set defaultPrevented(e){this[H]=e}preventDefault(){this.cancelable&&!this[H]&&(this[H]=!0)}};const me=Symbol("kEmitter"),Oe=Symbol("kBoundListener");var bs=class{constructor(e,r){this.socket=e,this.transport=r,this.id=ar(),this.url=new URL(e.url),this[me]=new EventTarget,this.transport.addEventListener("outgoing",s=>{const n=L(this.socket,new St("message",{data:s.data,origin:s.origin,cancelable:!0}));this[me].dispatchEvent(n),n.defaultPrevented&&s.preventDefault()}),this.transport.addEventListener("close",s=>{this[me].dispatchEvent(L(this.socket,new Ve("close",s)))})}addEventListener(e,r,s){if(!Reflect.has(r,Oe)){const n=r.bind(this.socket);Object.defineProperty(r,Oe,{value:n,enumerable:!1,configurable:!1})}this[me].addEventListener(e,Reflect.get(r,Oe),s)}removeEventListener(e,r,s){this[me].removeEventListener(e,Reflect.get(r,Oe),s)}send(e){this.transport.send(e)}close(e,r){this.transport.close(e,r)}};const It="InvalidAccessError: close code out of user configurable range",Je=Symbol("kPassthroughPromise"),pr=Symbol("kOnSend"),Ce=Symbol("kClose");var ks=class extends EventTarget{static{this.CONNECTING=0}static{this.OPEN=1}static{this.CLOSING=2}static{this.CLOSED=3}constructor(e,r){super(),this.CONNECTING=0,this.OPEN=1,this.CLOSING=2,this.CLOSED=3,this._onopen=null,this._onmessage=null,this._onerror=null,this._onclose=null,this.url=lr(e),this.protocol="",this.extensions="",this.binaryType="blob",this.readyState=this.CONNECTING,this.bufferedAmount=0,this[Je]=new Yr,queueMicrotask(async()=>{await this[Je]||(this.protocol=typeof r=="string"?r:Array.isArray(r)&&r.length>0?r[0]:"",this.readyState===this.CONNECTING&&(this.readyState=this.OPEN,this.dispatchEvent(L(this,new Event("open")))))})}set onopen(e){this.removeEventListener("open",this._onopen),this._onopen=e,e!==null&&this.addEventListener("open",e)}get onopen(){return this._onopen}set onmessage(e){this.removeEventListener("message",this._onmessage),this._onmessage=e,e!==null&&this.addEventListener("message",e)}get onmessage(){return this._onmessage}set onerror(e){this.removeEventListener("error",this._onerror),this._onerror=e,e!==null&&this.addEventListener("error",e)}get onerror(){return this._onerror}set onclose(e){this.removeEventListener("close",this._onclose),this._onclose=e,e!==null&&this.addEventListener("close",e)}get onclose(){return this._onclose}send(e){if(this.readyState===this.CONNECTING)throw this.close(),new DOMException("InvalidStateError");this.readyState===this.CLOSING||this.readyState===this.CLOSED||(this.bufferedAmount+=vs(e),queueMicrotask(()=>{this.bufferedAmount=0,this[pr]?.(e)}))}close(e=1e3,r){q(e,It),q(e===1e3||e>=3e3&&e<=4999,It),this[Ce](e,r)}[Ce](e=1e3,r,s=!0){this.readyState===this.CLOSING||this.readyState===this.CLOSED||(this.readyState=this.CLOSING,queueMicrotask(()=>{this.readyState=this.CLOSED,this.dispatchEvent(L(this,new Ve("close",{code:e,reason:r,wasClean:s}))),this._onopen=null,this._onmessage=null,this._onerror=null,this._onclose=null}))}addEventListener(e,r,s){return super.addEventListener(e,r,s)}removeEventListener(e,r,s){return super.removeEventListener(e,r,s)}};function vs(t){return typeof t=="string"?t.length:t instanceof Blob?t.size:t.byteLength}const Q=Symbol("kEmitter"),Ie=Symbol("kBoundListener"),nt=Symbol("kSend");var xs=class{constructor(e,r,s){this.client=e,this.transport=r,this.createConnection=s,this[Q]=new EventTarget,this.mockCloseController=new AbortController,this.realCloseController=new AbortController,this.transport.addEventListener("outgoing",n=>{typeof this.realWebSocket>"u"||queueMicrotask(()=>{n.defaultPrevented||this[nt](n.data)})}),this.transport.addEventListener("incoming",this.handleIncomingMessage.bind(this))}get socket(){return q(this.realWebSocket,'Cannot access "socket" on the original WebSocket server object: the connection is not open. Did you forget to call `server.connect()`?'),this.realWebSocket}connect(){q(!this.realWebSocket||this.realWebSocket.readyState!==WebSocket.OPEN,'Failed to call "connect()" on the original WebSocket instance: the connection already open');const e=this.createConnection();e.binaryType=this.client.binaryType,e.addEventListener("open",r=>{this[Q].dispatchEvent(L(this.realWebSocket,new Event("open",r)))},{once:!0}),e.addEventListener("message",r=>{this.transport.dispatchEvent(L(this.realWebSocket,new MessageEvent("incoming",{data:r.data,origin:r.origin})))}),this.client.addEventListener("close",r=>{this.handleMockClose(r)},{signal:this.mockCloseController.signal}),e.addEventListener("close",r=>{this.handleRealClose(r)},{signal:this.realCloseController.signal}),e.addEventListener("error",()=>{const r=L(e,new Event("error",{cancelable:!0}));this[Q].dispatchEvent(r),r.defaultPrevented||this.client.dispatchEvent(L(this.client,new Event("error")))}),this.realWebSocket=e}addEventListener(e,r,s){if(!Reflect.has(r,Ie)){const n=r.bind(this.client);Object.defineProperty(r,Ie,{value:n,enumerable:!1})}this[Q].addEventListener(e,Reflect.get(r,Ie),s)}removeEventListener(e,r,s){this[Q].removeEventListener(e,Reflect.get(r,Ie),s)}send(e){this[nt](e)}[nt](e){const{realWebSocket:r}=this;if(q(r,'Failed to call "server.send()" for "%s": the connection is not open. Did you forget to call "server.connect()"?',this.client.url),!(r.readyState===WebSocket.CLOSING||r.readyState===WebSocket.CLOSED)){if(r.readyState===WebSocket.CONNECTING){r.addEventListener("open",()=>{r.send(e)},{once:!0});return}r.send(e)}}close(){const{realWebSocket:e}=this;q(e,'Failed to close server connection for "%s": the connection is not open. Did you forget to call "server.connect()"?',this.client.url),this.realCloseController.abort(),!(e.readyState===WebSocket.CLOSING||e.readyState===WebSocket.CLOSED)&&(e.close(),queueMicrotask(()=>{this[Q].dispatchEvent(L(this.realWebSocket,new Ot("close",{code:1e3,cancelable:!0})))}))}handleIncomingMessage(e){const r=L(e.target,new St("message",{data:e.data,origin:e.origin,cancelable:!0}));this[Q].dispatchEvent(r),r.defaultPrevented||this.client.dispatchEvent(L(this.client,new MessageEvent("message",{data:e.data,origin:e.origin})))}handleMockClose(e){this.realWebSocket&&this.realWebSocket.close()}handleRealClose(e){this.mockCloseController.abort();const r=L(this.realWebSocket,new Ot("close",{code:e.code,reason:e.reason,wasClean:e.wasClean,cancelable:!0}));this[Q].dispatchEvent(r),r.defaultPrevented||this.client[Ce](e.code,e.reason)}},Ss=class extends EventTarget{constructor(e){super(),this.socket=e,this.socket.addEventListener("close",r=>{this.dispatchEvent(L(this.socket,new Ve("close",r)))}),this.socket[pr]=r=>{this.dispatchEvent(L(this.socket,new St("outgoing",{data:r,origin:this.socket.url,cancelable:!0})))}}addEventListener(e,r,s){return super.addEventListener(e,r,s)}dispatchEvent(e){return super.dispatchEvent(e)}send(e){queueMicrotask(()=>{if(this.socket.readyState===this.socket.CLOSING||this.socket.readyState===this.socket.CLOSED)return;const r=()=>{this.socket.dispatchEvent(L(this.socket,new MessageEvent("message",{data:e,origin:this.socket.url})))};this.socket.readyState===this.socket.CONNECTING?this.socket.addEventListener("open",()=>{r()},{once:!0}):r()})}close(e,r){this.socket[Ce](e,r)}};(class fr extends Br{static{this.symbol=Symbol.for("websocket-interceptor")}constructor(){super(fr.symbol)}checkEnvironment(){return ws("WebSocket")}setup(){const e=this.logger.extend("setup"),r=new Proxy(globalThis.WebSocket,{construct:(s,n,o)=>{const[i,a]=n,u=()=>Reflect.construct(s,n,o),d=new ks(i,a),m=new Ss(d);return queueMicrotask(async()=>{try{const p=new xs(d,m,u),f=this.emitter.listenerCount("connection")>0;await gs(this.emitter,"connection",{client:new bs(d,m),server:p,info:{protocols:a}}),f?d[Je].resolve(!1):(d[Je].resolve(!0),p.connect(),p.addEventListener("open",()=>{d.dispatchEvent(L(d,new Event("open"))),p.realWebSocket&&(d.protocol=p.realWebSocket.protocol)}))}catch(p){p instanceof Error&&(d.dispatchEvent(new Event("error")),d.readyState!==WebSocket.CLOSING&&d.readyState!==WebSocket.CLOSED&&d[Ce](1011,p.message,!1),console.error(p))}}),d}});e.info("patching global WebSocket..."),this.subscriptions.push(ys.applyPatch(globalThis,"WebSocket",()=>r)),e.info("global WebSocket patched!",globalThis.WebSocket.name)}});class _s extends ge{url;protocols;constructor(e,r){super(e,{}),this.url=r.url,this.protocols=r.protocols}}class Ls extends ge{url;protocols;error;constructor(e,r){super(e,{}),this.url=r.url,this.protocols=r.protocols,this.error=r.error}}class gr extends dr{constructor(e){super("ws",{connection:e.connection})}getHandlers(e){return e.getHandlersByKind("websocket")}async resolve(e,r,s){const{connection:n}=this.data;if(this.events.emit(new _s("connection",{url:n.client.url,protocols:n.info.protocols})),e.length===0)return await Ye(this,r).then(()=>this.passthrough(),i=>this.errorWith(i)),!1;let o=!1;for(const i of e){const a=await i.run(n,{baseUrl:s?.baseUrl?.toString(),[Xr]:!1});if(!a)continue;o=!0;const u=s?.quiet?void 0:i.log(n);try{i[Kr](a)||u?.()}catch(d){throw this.events.emit(new Ls("unhandledException",{error:d,url:n.client.url,protocols:n.info.protocols}))||(console.error(d),k.error('Encountered an unhandled exception during the handler lookup for "%s". Please see the original error above.',n.client.url)),d}}return o?!0:(await Ye(this,r).then(()=>this.passthrough(),i=>this.errorWith(i)),!1)}async getUnhandledMessage(){const{connection:e}=this.data;return`intercepted a WebSocket connection without a matching event handler:${`

  • ${e.client.url}

`}If you still wish to intercept this unhandled connection, please create an event handler for it.
Read more: https://mswjs.io/docs/websocket`}}class mr extends xt{#e;#t;constructor(e){super(),this.#e=new Fr({name:"interceptor-source",interceptors:e.interceptors}),this.#t=new Map}enable(){this.#e.apply(),this.#e.on("request",this.#s.bind(this)).on("response",this.#n.bind(this)).on("connection",this.#r.bind(this))}disable(){super.disable(),this.#e.dispose(),this.#t.clear()}async#s({requestId:e,request:r,controller:s}){const n=new Es({id:e,request:r,controller:s});this.#t.set(e,n),await this.queue(n)}async#n({requestId:e,request:r,response:s,isMockedResponse:n}){const o=this.#t.get(e);this.#t.delete(e),o!=null&&queueMicrotask(()=>{try{o.events.emit(new ur(n?"response:mocked":"response:bypass",{requestId:e,request:r,response:s}))}finally{o.events.removeAllListeners()}})}async#r(e){await this.queue(new qs({connection:e}))}}class Es extends Xe{#e;constructor(e){super({id:e.id,request:e.request}),this.#e=e.controller}passthrough(){hs(this.data.request)}respondWith(e){e&&this.#e.respondWith(e)}errorWith(e){if(e instanceof Response)return this.respondWith(e);throw e instanceof mt&&this.#e.errorWith(e),e}}class qs extends gr{constructor(e){super({connection:e.connection}),e.connection.client.addEventListener("close",()=>{this.events.removeAllListeners()},{once:!0})}errorWith(e){if(e instanceof Error){const{client:r}=this.data.connection,s=new Event("error");Object.defineProperty(s,"cause",{enumerable:!0,configurable:!1,value:e}),r.socket.dispatchEvent(s)}}passthrough(){this.data.connection.server.connect()}}function Cs(t){return({frame:e,defaults:r})=>{const s=t();if(s!=null){if(typeof s=="function"){const n=e instanceof Xe?e.data.request:e instanceof gr?new Request(e.data.connection.client.url,{headers:{connection:"upgrade",upgrade:"websocket"}}):null;return q(n!=null,'Failed to coerce a network frame to a legacy `onUnhandledRequest` strategy: unknown frame protocol "%s"',e.protocol),s(n,{warning:r.warn,error:r.error})}return Ye(e,s)}}}function js(t){return{status:t.status,statusText:t.statusText,headers:Object.fromEntries(t.headers.entries())}}var Rs={},Ps=/(%?)(%([sdijo]))/g;function Ms(t,e){switch(e){case"s":return t;case"d":case"i":return Number(t);case"j":return JSON.stringify(t);case"o":{if(typeof t=="string")return t;const r=JSON.stringify(t);return r==="{}"||r==="[]"||/^\[object .+?\]$/.test(r)?t:r}}}function Me(t,...e){if(e.length===0)return t;let r=0,s=t.replace(Ps,(n,o,i,a)=>{const u=e[r],d=Ms(u,a);return o?n:(r++,d)});return r<e.length&&(s+=` ${e.slice(r).join(" ")}`),s=s.replace(/%{2,2}/g,"%"),s}var Ts=2;function Os(t){if(!t.stack)return;const e=t.stack.split(`
`);e.splice(1,Ts),t.stack=e.join(`
`)}var Is=class extends Error{constructor(t,...e){super(t),this.message=t,this.name="Invariant Violation",this.message=Me(t,...e),Os(this)}},x=(t,e,...r)=>{if(!t)throw new Is(e,...r)};x.as=(t,e,r,...s)=>{if(!e){const n=s.length===0?r:Me(r,...s);let o;try{o=Reflect.construct(t,[n])}catch{o=t(n)}throw o}};function _t(){if(typeof navigator<"u"&&navigator.product==="ReactNative")return!0;if(typeof process<"u"){const t=process.type;return t==="renderer"||t==="worker"?!1:!!(process.versions&&process.versions.node)}return!1}var Ns=Object.defineProperty,Ws=(t,e)=>{for(var r in e)Ns(t,r,{get:e[r],enumerable:!0})},wt={};Ws(wt,{blue:()=>Ds,gray:()=>bt,green:()=>Hs,red:()=>Gs,yellow:()=>As});function As(t){return`\x1B[33m${t}\x1B[0m`}function Ds(t){return`\x1B[34m${t}\x1B[0m`}function bt(t){return`\x1B[90m${t}\x1B[0m`}function Gs(t){return`\x1B[31m${t}\x1B[0m`}function Hs(t){return`\x1B[32m${t}\x1B[0m`}var et=_t(),yr=class{constructor(t){this.name=t,this.prefix=`[${this.name}]`;const e=Nt("DEBUG"),r=Nt("LOG_LEVEL");e==="1"||e==="true"||typeof e<"u"&&this.name.startsWith(e)?(this.debug=ye(r,"debug")?I:this.debug,this.info=ye(r,"info")?I:this.info,this.success=ye(r,"success")?I:this.success,this.warning=ye(r,"warning")?I:this.warning,this.error=ye(r,"error")?I:this.error):(this.info=I,this.success=I,this.warning=I,this.error=I,this.only=I)}prefix;extend(t){return new yr(`${this.name}:${t}`)}debug(t,...e){this.logEntry({level:"debug",message:bt(t),positionals:e,prefix:this.prefix,colors:{prefix:"gray"}})}info(t,...e){this.logEntry({level:"info",message:t,positionals:e,prefix:this.prefix,colors:{prefix:"blue"}});const r=new Us;return(s,...n)=>{r.measure(),this.logEntry({level:"info",message:`${s} ${bt(`${r.deltaTime}ms`)}`,positionals:n,prefix:this.prefix,colors:{prefix:"blue"}})}}success(t,...e){this.logEntry({level:"info",message:t,positionals:e,prefix:`✔ ${this.prefix}`,colors:{timestamp:"green",prefix:"green"}})}warning(t,...e){this.logEntry({level:"warning",message:t,positionals:e,prefix:`⚠ ${this.prefix}`,colors:{timestamp:"yellow",prefix:"yellow"}})}error(t,...e){this.logEntry({level:"error",message:t,positionals:e,prefix:`✖ ${this.prefix}`,colors:{timestamp:"red",prefix:"red"}})}only(t){t()}createEntry(t,e){return{timestamp:new Date,level:t,message:e}}logEntry(t){const{level:e,message:r,prefix:s,colors:n,positionals:o=[]}=t,i=this.createEntry(e,r),a=n?.timestamp||"gray",u=n?.prefix||"gray",d={timestamp:wt[a],prefix:wt[u]};this.getWriter(e)([d.timestamp(this.formatTimestamp(i.timestamp))].concat(s!=null?d.prefix(s):[]).concat(Wt(r)).join(" "),...o.map(Wt))}formatTimestamp(t){return`${t.toLocaleTimeString("en-GB")}:${t.getMilliseconds()}`}getWriter(t){switch(t){case"debug":case"success":case"info":return $s;case"warning":return Bs;case"error":return Fs}}},Us=class{startTime;endTime;deltaTime;constructor(){this.startTime=performance.now()}measure(){this.endTime=performance.now();const t=this.endTime-this.startTime;this.deltaTime=t.toFixed(2)}},I=()=>{};function $s(t,...e){if(et){process.stdout.write(Me(t,...e)+`
`);return}console.log(t,...e)}function Bs(t,...e){if(et){process.stderr.write(Me(t,...e)+`
`);return}console.warn(t,...e)}function Fs(t,...e){if(et){process.stderr.write(Me(t,...e)+`
`);return}console.error(t,...e)}function Nt(t){return et?Rs[t]:globalThis[t]?.toString()}function ye(t,e){return t!==void 0&&t!==e}function Wt(t){return typeof t>"u"?"undefined":t===null?"null":typeof t=="string"?t:typeof t=="object"?JSON.stringify(t):t.toString()}var Qs=class extends Error{constructor(t,e,r){super(`Possible EventEmitter memory leak detected. ${r} ${e.toString()} listeners added. Use emitter.setMaxListeners() to increase limit`),this.emitter=t,this.type=e,this.count=r,this.name="MaxListenersExceededWarning"}},wr=class{static listenerCount(t,e){return t.listenerCount(e)}constructor(){this.events=new Map,this.maxListeners=wr.defaultMaxListeners,this.hasWarnedAboutPotentialMemoryLeak=!1}_emitInternalEvent(t,e,r){this.emit(t,e,r)}_getListeners(t){return Array.prototype.concat.apply([],this.events.get(t))||[]}_removeListener(t,e){const r=t.indexOf(e);return r>-1&&t.splice(r,1),[]}_wrapOnceListener(t,e){const r=(...s)=>(this.removeListener(t,r),e.apply(this,s));return Object.defineProperty(r,"name",{value:e.name}),r}setMaxListeners(t){return this.maxListeners=t,this}getMaxListeners(){return this.maxListeners}eventNames(){return Array.from(this.events.keys())}emit(t,...e){const r=this._getListeners(t);return r.forEach(s=>{s.apply(this,e)}),r.length>0}addListener(t,e){this._emitInternalEvent("newListener",t,e);const r=this._getListeners(t).concat(e);if(this.events.set(t,r),this.maxListeners>0&&this.listenerCount(t)>this.maxListeners&&!this.hasWarnedAboutPotentialMemoryLeak){this.hasWarnedAboutPotentialMemoryLeak=!0;const s=new Qs(this,t,this.listenerCount(t));console.warn(s)}return this}on(t,e){return this.addListener(t,e)}once(t,e){return this.addListener(t,this._wrapOnceListener(t,e))}prependListener(t,e){const r=this._getListeners(t);if(r.length>0){const s=[e].concat(r);this.events.set(t,s)}else this.events.set(t,r.concat(e));return this}prependOnceListener(t,e){return this.prependListener(t,this._wrapOnceListener(t,e))}removeListener(t,e){const r=this._getListeners(t);return r.length>0&&(this._removeListener(r,e),this.events.set(t,r),this._emitInternalEvent("removeListener",t,e)),this}off(t,e){return this.removeListener(t,e)}removeAllListeners(t){return t?this.events.delete(t):this.events.clear(),this}listeners(t){return Array.from(this._getListeners(t))}listenerCount(t){return this._getListeners(t).length}rawListeners(t){return this.listeners(t)}},br=wr;br.defaultMaxListeners=10;var zs="x-interceptors-internal-request-id";function At(t){return globalThis[t]||void 0}function Zs(t,e){globalThis[t]=e}function Ys(t){delete globalThis[t]}var N=(function(t){return t.INACTIVE="INACTIVE",t.APPLYING="APPLYING",t.APPLIED="APPLIED",t.DISPOSING="DISPOSING",t.DISPOSED="DISPOSED",t})({}),Lt=class{constructor(t){this.symbol=t,this.readyState=N.INACTIVE,this.emitter=new br,this.subscriptions=[],this.logger=new yr(t.description),this.emitter.setMaxListeners(0),this.logger.info("constructing the interceptor...")}checkEnvironment(){return!0}apply(){const t=this.logger.extend("apply");if(t.info("applying the interceptor..."),this.readyState===N.APPLIED){t.info("intercepted already applied!");return}if(!this.checkEnvironment()){t.info("the interceptor cannot be applied in this environment!");return}this.readyState=N.APPLYING;const e=this.getInstance();if(e){t.info("found a running instance, reusing..."),this.on=(r,s)=>(t.info('proxying the "%s" listener',r),e.emitter.addListener(r,s),this.subscriptions.push(()=>{e.emitter.removeListener(r,s),t.info('removed proxied "%s" listener!',r)}),this),this.readyState=N.APPLIED;return}t.info("no running instance found, setting up a new instance..."),this.setup(),this.setInstance(),this.readyState=N.APPLIED}setup(){}on(t,e){const r=this.logger.extend("on");return this.readyState===N.DISPOSING||this.readyState===N.DISPOSED?(r.info("cannot listen to events, already disposed!"),this):(r.info('adding "%s" event listener:',t,e),this.emitter.on(t,e),this)}once(t,e){return this.emitter.once(t,e),this}off(t,e){return this.emitter.off(t,e),this}removeAllListeners(t){return this.emitter.removeAllListeners(t),this}dispose(){const t=this.logger.extend("dispose");if(this.readyState===N.DISPOSED){t.info("cannot dispose, already disposed!");return}if(t.info("disposing the interceptor..."),this.readyState=N.DISPOSING,!this.getInstance()){t.info("no interceptors running, skipping dispose...");return}if(this.clearInstance(),t.info("global symbol deleted:",At(this.symbol)),this.subscriptions.length>0){t.info("disposing of %d subscriptions...",this.subscriptions.length);for(const e of this.subscriptions)e();this.subscriptions=[],t.info("disposed of all subscriptions!",this.subscriptions.length)}this.emitter.removeAllListeners(),t.info("destroyed the listener!"),this.readyState=N.DISPOSED}getInstance(){const t=At(this.symbol);return this.logger.info("retrieved global instance:",t?.constructor?.name),t}setInstance(){Zs(this.symbol,this),this.logger.info("set global instance!",this.symbol.description)}clearInstance(){Ys(this.symbol),this.logger.info("cleared global instance!",this.symbol.description)}};function Et(){return Math.random().toString(16).slice(2)}function kr(t){if(typeof t=="string")return kr(new URL(t,typeof location<"u"?location.href:void 0));if(t.protocol==="http:"?t.protocol="ws:":t.protocol==="https:"&&(t.protocol="wss:"),t.protocol!=="ws:"&&t.protocol!=="wss:")throw new SyntaxError(`Failed to construct 'WebSocket': The URL's scheme must be either 'http', 'https', 'ws', or 'wss'. '${t.protocol}' is not allowed.`);if(t.hash!=="")throw new SyntaxError(`Failed to construct 'WebSocket': The URL contains a fragment identifier ('${t.hash}'). Fragment identifiers are not allowed in WebSocket URLs.`);return t.href}async function je(t,e,...r){const s=t.listeners(e);if(s.length!==0)for(const n of s)await n.apply(t,r)}function qt(t){const e=Object.getOwnPropertyDescriptor(globalThis,t);return typeof e>"u"||typeof e.get=="function"&&typeof e.get()>"u"||typeof e.get>"u"&&e.value==null?!1:typeof e.set>"u"&&!e.configurable?(console.error(`[MSW] Failed to apply interceptor: the global \`${t}\` property is non-configurable. This is likely an issue with your environment. If you are using a framework, please open an issue about this in their repository.`),!1):!0}function Js(){const t=(e,r)=>{t.state="pending",t.resolve=s=>{if(t.state!=="pending")return;t.result=s;const n=o=>(t.state="fulfilled",o);return e(s instanceof Promise?s:Promise.resolve(s).then(n))},t.reject=s=>{if(t.state==="pending")return queueMicrotask(()=>{t.state="rejected"}),r(t.rejectionReason=s)}};return t}var tt=class extends Promise{#e;resolve;reject;constructor(t=null){const e=Js();super((r,s)=>{e(r,s),t?.(e.resolve,e.reject)}),this.#e=e,this.resolve=this.#e.resolve,this.reject=this.#e.reject}get state(){return this.#e.state}get rejectionReason(){return this.#e.rejectionReason}then(t,e){return this.#t(super.then(t,e))}catch(t){return this.#t(super.catch(t))}finally(t){return this.#t(super.finally(t))}#t(t){return Object.defineProperties(t,{resolve:{configurable:!0,value:this.resolve},reject:{configurable:!0,value:this.reject}})}};function E(t,e){return Object.defineProperties(e,{target:{value:t,enumerable:!0,writable:!0},currentTarget:{value:t,enumerable:!0,writable:!0}}),e}var he=Symbol("kCancelable"),U=Symbol("kDefaultPrevented"),Ct=class extends MessageEvent{constructor(t,e){super(t,e),this[he]=!!e.cancelable,this[U]=!1}get cancelable(){return this[he]}set cancelable(t){this[he]=t}get defaultPrevented(){return this[U]}set defaultPrevented(t){this[U]=t}preventDefault(){this.cancelable&&!this[U]&&(this[U]=!0)}},rt=class extends Event{constructor(t,e={}){super(t,e),this.code=e.code===void 0?0:e.code,this.reason=e.reason===void 0?"":e.reason,this.wasClean=e.wasClean===void 0?!1:e.wasClean}},Dt=class extends rt{constructor(t,e={}){super(t,e),this[he]=!!e.cancelable,this[U]=!1}get cancelable(){return this[he]}set cancelable(t){this[he]=t}get defaultPrevented(){return this[U]}set defaultPrevented(t){this[U]=t}preventDefault(){this.cancelable&&!this[U]&&(this[U]=!0)}},we=Symbol("kEmitter"),Ne=Symbol("kBoundListener"),Ks=class{constructor(t,e){this.socket=t,this.transport=e,this.id=Et(),this.url=new URL(t.url),this[we]=new EventTarget,this.transport.addEventListener("outgoing",r=>{const s=E(this.socket,new Ct("message",{data:r.data,origin:r.origin,cancelable:!0}));this[we].dispatchEvent(s),s.defaultPrevented&&r.preventDefault()}),this.transport.addEventListener("close",r=>{this[we].dispatchEvent(E(this.socket,new rt("close",r)))})}addEventListener(t,e,r){if(!Reflect.has(e,Ne)){const s=e.bind(this.socket);Object.defineProperty(e,Ne,{value:s,enumerable:!1,configurable:!1})}this[we].addEventListener(t,Reflect.get(e,Ne),r)}removeEventListener(t,e,r){this[we].removeEventListener(t,Reflect.get(e,Ne),r)}send(t){this.transport.send(t)}close(t,e){this.transport.close(t,e)}},Gt="InvalidAccessError: close code out of user configurable range",Ke=Symbol("kPassthroughPromise"),vr=Symbol("kOnSend"),Re=Symbol("kClose"),Xs=class extends EventTarget{static{this.CONNECTING=0}static{this.OPEN=1}static{this.CLOSING=2}static{this.CLOSED=3}constructor(t,e){super(),this.CONNECTING=0,this.OPEN=1,this.CLOSING=2,this.CLOSED=3,this._onopen=null,this._onmessage=null,this._onerror=null,this._onclose=null,this.url=kr(t),this.protocol="",this.extensions="",this.binaryType="blob",this.readyState=this.CONNECTING,this.bufferedAmount=0,this[Ke]=new tt,queueMicrotask(async()=>{await this[Ke]||(this.protocol=typeof e=="string"?e:Array.isArray(e)&&e.length>0?e[0]:"",this.readyState===this.CONNECTING&&(this.readyState=this.OPEN,this.dispatchEvent(E(this,new Event("open")))))})}set onopen(t){this.removeEventListener("open",this._onopen),this._onopen=t,t!==null&&this.addEventListener("open",t)}get onopen(){return this._onopen}set onmessage(t){this.removeEventListener("message",this._onmessage),this._onmessage=t,t!==null&&this.addEventListener("message",t)}get onmessage(){return this._onmessage}set onerror(t){this.removeEventListener("error",this._onerror),this._onerror=t,t!==null&&this.addEventListener("error",t)}get onerror(){return this._onerror}set onclose(t){this.removeEventListener("close",this._onclose),this._onclose=t,t!==null&&this.addEventListener("close",t)}get onclose(){return this._onclose}send(t){if(this.readyState===this.CONNECTING)throw this.close(),new DOMException("InvalidStateError");this.readyState===this.CLOSING||this.readyState===this.CLOSED||(this.bufferedAmount+=Vs(t),queueMicrotask(()=>{this.bufferedAmount=0,this[vr]?.(t)}))}close(t=1e3,e){x(t,Gt),x(t===1e3||t>=3e3&&t<=4999,Gt),this[Re](t,e)}[Re](t=1e3,e,r=!0){this.readyState===this.CLOSING||this.readyState===this.CLOSED||(this.readyState=this.CLOSING,queueMicrotask(()=>{this.readyState=this.CLOSED,this.dispatchEvent(E(this,new rt("close",{code:t,reason:e,wasClean:r}))),this._onopen=null,this._onmessage=null,this._onerror=null,this._onclose=null}))}addEventListener(t,e,r){return super.addEventListener(t,e,r)}removeEventListener(t,e,r){return super.removeEventListener(t,e,r)}};function Vs(t){return typeof t=="string"?t.length:t instanceof Blob?t.size:t.byteLength}var z=Symbol("kEmitter"),We=Symbol("kBoundListener"),ot=Symbol("kSend"),en=class{constructor(t,e,r){this.client=t,this.transport=e,this.createConnection=r,this[z]=new EventTarget,this.mockCloseController=new AbortController,this.realCloseController=new AbortController,this.transport.addEventListener("outgoing",s=>{typeof this.realWebSocket>"u"||queueMicrotask(()=>{s.defaultPrevented||this[ot](s.data)})}),this.transport.addEventListener("incoming",this.handleIncomingMessage.bind(this))}get socket(){return x(this.realWebSocket,'Cannot access "socket" on the original WebSocket server object: the connection is not open. Did you forget to call `server.connect()`?'),this.realWebSocket}connect(){x(!this.realWebSocket||this.realWebSocket.readyState!==WebSocket.OPEN,'Failed to call "connect()" on the original WebSocket instance: the connection already open');const t=this.createConnection();t.binaryType=this.client.binaryType,t.addEventListener("open",e=>{this[z].dispatchEvent(E(this.realWebSocket,new Event("open",e)))},{once:!0}),t.addEventListener("message",e=>{this.transport.dispatchEvent(E(this.realWebSocket,new MessageEvent("incoming",{data:e.data,origin:e.origin})))}),this.client.addEventListener("close",e=>{this.handleMockClose(e)},{signal:this.mockCloseController.signal}),t.addEventListener("close",e=>{this.handleRealClose(e)},{signal:this.realCloseController.signal}),t.addEventListener("error",()=>{const e=E(t,new Event("error",{cancelable:!0}));this[z].dispatchEvent(e),e.defaultPrevented||this.client.dispatchEvent(E(this.client,new Event("error")))}),this.realWebSocket=t}addEventListener(t,e,r){if(!Reflect.has(e,We)){const s=e.bind(this.client);Object.defineProperty(e,We,{value:s,enumerable:!1})}this[z].addEventListener(t,Reflect.get(e,We),r)}removeEventListener(t,e,r){this[z].removeEventListener(t,Reflect.get(e,We),r)}send(t){this[ot](t)}[ot](t){const{realWebSocket:e}=this;if(x(e,'Failed to call "server.send()" for "%s": the connection is not open. Did you forget to call "server.connect()"?',this.client.url),!(e.readyState===WebSocket.CLOSING||e.readyState===WebSocket.CLOSED)){if(e.readyState===WebSocket.CONNECTING){e.addEventListener("open",()=>{e.send(t)},{once:!0});return}e.send(t)}}close(){const{realWebSocket:t}=this;x(t,'Failed to close server connection for "%s": the connection is not open. Did you forget to call "server.connect()"?',this.client.url),this.realCloseController.abort(),!(t.readyState===WebSocket.CLOSING||t.readyState===WebSocket.CLOSED)&&(t.close(),queueMicrotask(()=>{this[z].dispatchEvent(E(this.realWebSocket,new Dt("close",{code:1e3,cancelable:!0})))}))}handleIncomingMessage(t){const e=E(t.target,new Ct("message",{data:t.data,origin:t.origin,cancelable:!0}));this[z].dispatchEvent(e),e.defaultPrevented||this.client.dispatchEvent(E(this.client,new MessageEvent("message",{data:t.data,origin:t.origin})))}handleMockClose(t){this.realWebSocket&&this.realWebSocket.close()}handleRealClose(t){this.mockCloseController.abort();const e=E(this.realWebSocket,new Dt("close",{code:t.code,reason:t.reason,wasClean:t.wasClean,cancelable:!0}));this[z].dispatchEvent(e),e.defaultPrevented||this.client[Re](t.code,t.reason)}},tn=class extends EventTarget{constructor(t){super(),this.socket=t,this.socket.addEventListener("close",e=>{this.dispatchEvent(E(this.socket,new rt("close",e)))}),this.socket[vr]=e=>{this.dispatchEvent(E(this.socket,new Ct("outgoing",{data:e,origin:this.socket.url,cancelable:!0})))}}addEventListener(t,e,r){return super.addEventListener(t,e,r)}dispatchEvent(t){return super.dispatchEvent(t)}send(t){queueMicrotask(()=>{if(this.socket.readyState===this.socket.CLOSING||this.socket.readyState===this.socket.CLOSED)return;const e=()=>{this.socket.dispatchEvent(E(this.socket,new MessageEvent("message",{data:t,origin:this.socket.url})))};this.socket.readyState===this.socket.CONNECTING?this.socket.addEventListener("open",()=>{e()},{once:!0}):e()})}close(t,e){this.socket[Re](t,e)}},rn=class xr extends Lt{static{this.symbol=Symbol("websocket")}constructor(){super(xr.symbol)}checkEnvironment(){return qt("WebSocket")}setup(){const e=Object.getOwnPropertyDescriptor(globalThis,"WebSocket"),r=new Proxy(globalThis.WebSocket,{construct:(s,n,o)=>{const[i,a]=n,u=()=>Reflect.construct(s,n,o),d=new Xs(i,a),m=new tn(d);return queueMicrotask(async()=>{try{const p=new en(d,m,u),f=this.emitter.listenerCount("connection")>0;await je(this.emitter,"connection",{client:new Ks(d,m),server:p,info:{protocols:a}}),f?d[Ke].resolve(!1):(d[Ke].resolve(!0),p.connect(),p.addEventListener("open",()=>{d.dispatchEvent(E(d,new Event("open"))),p.realWebSocket&&(d.protocol=p.realWebSocket.protocol)}))}catch(p){p instanceof Error&&(d.dispatchEvent(new Event("error")),d.readyState!==WebSocket.CLOSING&&d.readyState!==WebSocket.CLOSED&&d[Re](1011,p.message,!1),console.error(p))}}),d}});Object.defineProperty(globalThis,"WebSocket",{value:r,configurable:!0}),this.subscriptions.push(()=>{Object.defineProperty(globalThis,"WebSocket",e)})}};function jt(){return typeof navigator<"u"&&"serviceWorker"in navigator&&typeof location<"u"&&location.protocol!=="file:"}function sn(){try{const t=new ReadableStream({start:r=>r.close()});return new MessageChannel().port1.postMessage(t,[t]),!0}catch{return!1}}function nn(){const t=((e,r)=>{t.state="pending",t.resolve=s=>{if(t.state!=="pending")return;t.result=s;const n=o=>(t.state="fulfilled",o);return e(s instanceof Promise?s:Promise.resolve(s).then(n))},t.reject=s=>{if(t.state==="pending")return queueMicrotask(()=>{t.state="rejected"}),r(t.rejectionReason=s)}});return t}var be=class extends Promise{#e;resolve;reject;constructor(t=null){const e=nn();super((r,s)=>{e(r,s),t?.(e.resolve,e.reject)}),this.#e=e,this.resolve=this.#e.resolve,this.reject=this.#e.reject}get state(){return this.#e.state}get rejectionReason(){return this.#e.rejectionReason}then(t,e){return this.#t(super.then(t,e))}catch(t){return this.#t(super.catch(t))}finally(t){return this.#t(super.finally(t))}#t(t){return Object.defineProperties(t,{resolve:{configurable:!0,value:this.resolve},reject:{configurable:!0,value:this.reject}})}},pe=Symbol("isPatchedModule"),$e=class Sr extends Error{constructor(e){super(e),this.name="InterceptorError",Object.setPrototypeOf(this,Sr.prototype)}},Le=class Y{static{this.PENDING=0}static{this.PASSTHROUGH=1}static{this.RESPONSE=2}static{this.ERROR=3}constructor(e,r){this.request=e,this.source=r,this.readyState=Y.PENDING,this.handled=new tt}get#e(){return this.handled}async passthrough(){x.as($e,this.readyState===Y.PENDING,'Failed to passthrough the "%s %s" request: the request has already been handled',this.request.method,this.request.url),this.readyState=Y.PASSTHROUGH,await this.source.passthrough(),this.#e.resolve()}respondWith(e){x.as($e,this.readyState===Y.PENDING,'Failed to respond to the "%s %s" request with "%d %s": the request has already been handled (%d)',this.request.method,this.request.url,e.status,e.statusText||"OK",this.readyState),this.readyState=Y.RESPONSE,this.#e.resolve(),this.source.respondWith(e)}errorWith(e){x.as($e,this.readyState===Y.PENDING,'Failed to error the "%s %s" request with "%s": the request has already been handled (%d)',this.request.method,this.request.url,e?.toString(),this.readyState),this.readyState=Y.ERROR,this.source.errorWith(e),this.#e.resolve()}};function _r(t){try{return new URL(t),!0}catch{return!1}}function Ht(t,e){const r=Object.getOwnPropertySymbols(e).find(s=>s.description===t);if(r)return Reflect.get(e,r)}var re=class le extends Response{static{this.STATUS_CODES_WITHOUT_BODY=[101,103,204,205,304]}static{this.STATUS_CODES_WITH_REDIRECT=[301,302,303,307,308]}static isConfigurableStatusCode(e){return e>=200&&e<=599}static isRedirectResponse(e){return le.STATUS_CODES_WITH_REDIRECT.includes(e)}static isResponseWithBody(e){return!le.STATUS_CODES_WITHOUT_BODY.includes(e)}static setUrl(e,r){if(!e||e==="about:"||!_r(e))return;const s=Ht("state",r);s?s.urlList.push(new URL(e)):Object.defineProperty(r,"url",{value:e,enumerable:!0,configurable:!0,writable:!1})}static parseRawHeaders(e){const r=new Headers;for(let s=0;s<e.length;s+=2)r.append(e[s],e[s+1]);return r}constructor(e,r={}){const s=r.status??200,n=le.isConfigurableStatusCode(s)?s:200,o=le.isResponseWithBody(s)?e:null;if(super(o,{status:n,statusText:r.statusText,headers:r.headers}),s!==n){const i=Ht("state",this);i?i.status=s:Object.defineProperty(this,"status",{value:s,enumerable:!0,configurable:!0,writable:!1})}le.setUrl(r.url,this)}},on=Symbol("kRawRequest");function Lr(t,e){Reflect.set(t,on,e)}var an=new TextEncoder;function ln(t){return an.encode(t)}function cn(t,e){return new TextDecoder(e).decode(t)}function dn(t){return t.buffer.slice(t.byteOffset,t.byteOffset+t.byteLength)}async function un(t){try{return[null,await t().catch(e=>{throw e})]}catch(e){return[e,null]}}function hn(t){return new URL(t,location.href).href}function it(t,e,r){return[t.active,t.installing,t.waiting].filter(i=>i!=null).find(i=>r(i.scriptURL,e))||null}var pn=async(t,e={},r)=>{const s=hn(t),n=await navigator.serviceWorker.getRegistrations().then(u=>u.filter(d=>it(d,s,r)));!navigator.serviceWorker.controller&&n.length>0&&location.reload();const[o]=n;if(o)return o.update(),[it(o,s,r),o];const[i,a]=await un(async()=>{const u=await navigator.serviceWorker.register(t,e);return[it(u,s,r),u]});if(i){if(i.message.includes("(404)")){const d=new URL(e?.scope||"/",location.href);throw new Error(k.formatMessage(`Failed to register a Service Worker for scope ('${d.href}') with script ('${s}'): Service Worker script does not exist at the given path.
...
%s`,i.message))}return a},Ut=class{#e;#t;constructor(){this.#e=[],this.#t=new Map}get[Symbol.iterator](){return this.#e[Symbol.iterator].bind(this.#e)}entries(){return this.#t.entries()}get(t){return this.#t.get(t)||[]}getAll(){return this.#e.map(([,t])=>t)}append(t,e){this.#e.push([t,e]),this.#s(t,r=>r.push(e))}prepend(t,e){this.#e.unshift([t,e]),this.#s(t,r=>r.unshift(e))}delete(t,e){if(this.size===0)return!1;const r=this.#t.get(t);if(!r)return!1;const s=r.indexOf(e);return s===-1?!1:(r.splice(s,1),this.#e.splice(this.#e.findIndex(n=>n[0]===t&&n[1]===e),1),!0)}deleteAll(t){this.size!==0&&(this.#e=this.#e.filter(e=>e[0]!==t),this.#t.delete(t))}get size(){return this.#e.length}clear(){this.size!==0&&(this.#e.length=0,this.#t.clear())}#s(t,e){e(this.#t.get(t)||this.#t.set(t,[]).get(t))}},Ae=Symbol("kDefaultPrevented"),J=Symbol("kPropagationStopped"),Ee=Symbol("kImmediatePropagationStopped"),fn=class extends MessageEvent{#e;[Ae];[J];[Ee];constructor(...t){super(t[0],t[1]),this[Ae]=!1}get defaultPrevented(){return this[Ae]}preventDefault(){super.preventDefault(),this[Ae]=!0}stopImmediatePropagation(){super.stopImmediatePropagation(),this[Ee]=!0}},gn=class{#e;#t;#s;#n;#r;#a;#o;hooks;constructor(){this.#e=new Ut,this.#t=new WeakMap,this.#s=new WeakMap,this.#n=new WeakSet,this.#r=new Ut,this.#a=new WeakMap,this.#o=new WeakMap,this.hooks={on:(t,e,r)=>{if(!r?.signal?.aborted){if(r?.once){const s=e,n=((...o)=>(this.#i(t,n),s(...o)));e=n}if(this.#r.append(t,e),r&&this.#a.set(e,r),r?.signal){const{signal:s}=r,n=()=>{this.#i(t,e)};s.addEventListener("abort",n,{once:!0}),this.#o.set(e,()=>{s.removeEventListener("abort",n)})}}},removeListener:(t,e)=>{this.#i(t,e)}}}#i(t,e){this.#r.delete(t,e);const r=this.#o.get(e);r&&(r(),this.#o.delete(e))}#h(t,e){const r=this.#e.delete(t,e),s=this.#s.get(e);return s&&(s(),this.#s.delete(e)),r}on(t,e,r){return this.#p(t,e,r),this}once(t,e,r){return this.on(t,e,{...r||{},once:!0})}earlyOn(t,e,r){return this.#p(t,e,r,"prepend"),this}earlyOnce(t,e,r){return this.earlyOn(t,e,{...r||{},once:!0})}emit(t){if(this.#e.size===0)return!1;const e=this.listenerCount(t.type)>0,r=this.#l(t);for(const s of this.#d(t.type)){if(r.event[J]!=null&&r.event[J]!==this)return r.revoke(),!1;if(r.event[Ee])break;this.#c(r.event,s)}return r.revoke(),e}async emitAsPromise(t){if(this.#e.size===0)return[];const e=[],r=this.#l(t);for(const s of this.#d(t.type)){if(r.event[J]!=null&&r.event[J]!==this)return r.revoke(),[];if(r.event[Ee])break;const n=await Promise.resolve(this.#c(r.event,s));this.#u(s)||e.push(n)}return r.revoke(),Promise.allSettled(e).then(s=>s.map(n=>n.status==="fulfilled"?n.value:n.reason))}*emitAsGenerator(t){if(this.#e.size===0)return;const e=this.#l(t);for(const r of this.#d(t.type)){if(e.event[J]!=null&&e.event[J]!==this){e.revoke();return}if(e.event[Ee])break;const s=this.#c(e.event,r);this.#u(r)||(yield s)}e.revoke()}removeListener(t,e){const r=this.#t.get(e);if(this.#h(t,e))for(const s of this.#r.get("removeListener").slice())s(t,e,r)}removeAllListeners(t){if(t==null){for(const[r,s]of this.#e.entries())for(;s.length>0;)this.removeListener(r,s[0]);for(const[r,s]of[...this.#r])this.#a.get(s)?.persist||this.#i(r,s);return}const e=this.listeners(t);for(;e.length>0;)this.removeListener(t,e[0])}listeners(t){return t==null?this.#e.getAll():this.#e.get(t)}listenerCount(t){return t==null?this.#e.size:this.listeners(t).length}#p(t,e,r,s="append"){if(!r?.signal?.aborted){for(const n of this.#r.get("newListener").slice())n(t,e,r);if(t==="*"&&this.#n.add(e),s==="prepend"?this.#e.prepend(t,e):this.#e.append(t,e),r&&(this.#t.set(e,r),r.signal)){const{signal:n}=r,o=()=>{this.removeListener(t,e)};n.addEventListener("abort",o,{once:!0}),this.#s.set(e,()=>{n.removeEventListener("abort",o)})}}}#l(t){const{stopPropagation:e}=t;return t.stopPropagation=()=>{t[J]=this,e.call(t)},{event:t,revoke(){t.stopPropagation=e}}}#c(t,e){for(const n of this.#r.get("beforeEmit").slice())if(n(t)===!1)return;const r=e.call(this,t),s=this.#t.get(e);if(s?.once){const n=this.#u(e)?"*":t.type;if(this.#h(n,e))for(const o of this.#r.get("removeListener").slice())o(n,e,s)}return r}*#d(t){const e=[];for(const[r,s]of this.#e)(r==="*"||r===t)&&e.push(s);yield*e}#u(t){return this.#n.has(t)}},$t=jt(),mn=class extends fn{#e;constructor(t){const e=t.data.type,r=t.data.payload;super(e,{data:r}),this.#e=t}get ports(){return this.#e.ports}postMessage(t,...e){this.#e.ports[0].postMessage({type:t,data:e[0]},{transfer:e[1]})}},yn=class extends gn{#e;#t;constructor(t){super(),x($t,"Failed to open a WorkerChannel: Service Worker is not supported in this environment."),this.#e=t.getWorker,this.#t=new AbortController,navigator.serviceWorker.addEventListener("message",async e=>{const r=await this.#e();e.source!=null&&e.source!==r||e.data&&Jr(e.data)&&"type"in e.data&&this.emit(new mn(e))},{signal:this.#t.signal})}postMessage(t){x($t,"Failed to post message on a WorkerChannel: the Service Worker API is unavailable in this environment. This is likely an issue with MSW. Please report it on GitHub: https://github.com/mswjs/msw/issues"),this.#e().then(e=>{e.postMessage(t)})}terminate(){this.#t.abort(),this.removeAllListeners()}};function wn(t){if(!["HEAD","GET"].includes(t.method))return t.body}function Bt(t){return new Request(t.url,{...t,body:wn(t)})}function bn(t){location.href.startsWith(t.scope)||k.warn(`Cannot intercept requests on this page because it's outside of the worker's scope ("${t.scope}"). If you wish to mock API requests on this page, you must resolve this scope issue.

- (Recommended) Register the worker at the root level ("/") of your application.
- Set the "Service-Worker-Allowed" response header to allow out-of-scope workers.`)}function kn(t,e){return t.findWorker!==e.findWorker||t.serviceWorker.url!==e.serviceWorker.url||JSON.stringify(t.serviceWorker.options)!==JSON.stringify(e.serviceWorker.options)}var Ft=class M extends xt{static#e;static async from(e){return M.#e==null?M.#e=new M(e):kn(M.#e.#t,e)&&(await M.#e.terminate(),M.#e=new M(e)),M.#e}#t;#s;#n;#r;#a;#o;#i;workerPromise;constructor(e){super(),x(jt(),"Failed to use Service Worker as the network source: the Service Worker API is not supported in this environment"),this.#t=e,this.#s=new Map,this.workerPromise=new be,this.#n=new yn({getWorker:()=>this.workerPromise.then(([r])=>r)})}async enable(){if(this.workerPromise.state==="fulfilled"&&typeof this.#i>"u")return k.warn('Found a redundant "worker.start()" call. Note that starting the worker while mocking is already enabled will have no effect. Consider removing this "worker.start()" call.'),this.workerPromise.then(([,n])=>n);this.#i=void 0,this.#n.removeAllListeners(),this.#s.clear(),this.#r=new AbortController;const[e,r]=await this.#h();if(e.state!=="activated"){const n=new AbortController,o=new be;o.then(()=>n.abort()),e.addEventListener("statechange",()=>{e.state==="activated"&&o.resolve()},{signal:n.signal}),await o}this.#n.postMessage("MOCK_ACTIVATE");const s=new be;return this.#a=s,this.#n.once("MOCKING_ENABLED",n=>{s.resolve(n.data.client)}),await s,this.#t.quiet||this.#u(),r}disable(){if(typeof this.#i<"u"){k.warn('Found a redundant "worker.stop()" call. Notice that stopping the worker after it has already been stopped has no effect. Consider removing this "worker.stop()" call.');return}this.#i=Date.now(),this.#r?.abort(),this.#r=void 0,this.#n.postMessage("CLIENT_CLOSED"),this.#t.quiet||this.#f()}async terminate(){if(this.#o!=null&&(clearInterval(this.#o),this.#o=void 0),this.#s.clear(),this.#n.terminate(),this.#r?.abort(),this.#r=void 0,this.workerPromise.state==="fulfilled"){const[,e]=await this.workerPromise;await e.unregister()}M.#e===this&&(M.#e=void 0)}async#h(){this.#o&&clearInterval(this.#o);const e=this.#t.serviceWorker.url,[r,s]=await pn(e,this.#t.serviceWorker.options,this.#t.findWorker||this.#c);if(r==null){const n=this.#t?.findWorker?k.formatMessage(`Failed to locate the Service Worker registration using a custom "findWorker" predicate.

Please ensure that the custom predicate properly locates the Service Worker registration at "%s".
More details: https://mswjs.io/docs/api/setup-worker/start#findworker
     `,e):k.formatMessage(`Failed to locate the Service Worker registration.

This most likely means that the worker script URL "%s" cannot resolve against the actual public hostname (%s). This may happen if your application runs behind a proxy, or has a dynamic hostname.

Please consider using a custom "serviceWorker.url" option to point to the actual worker script location, or a custom "findWorker" option to resolve the Service Worker registration manually. More details: https://mswjs.io/docs/api/setup-worker/start`,e,location.host);throw new Error(n)}return this.workerPromise.state==="pending"?this.workerPromise.resolve([r,s]):this.workerPromise=new be(n=>{n([r,s])}),this.#n.on("REQUEST",this.#p.bind(this)),this.#n.on("RESPONSE",this.#l.bind(this)),window.addEventListener("beforeunload",()=>{r.state!=="redundant"&&this.#n.postMessage("CLIENT_CLOSED"),clearInterval(this.#o),window.postMessage({type:"msw/worker:stop"})},{signal:this.#r?.signal}),await this.#d().catch(n=>{k.error("Error while checking the worker script integrity. Please report this on GitHub (https://github.com/mswjs/msw/issues) and include the original error below."),console.error(n)}),this.#o=window.setInterval(()=>{this.#n.postMessage("KEEPALIVE_REQUEST")},5e3),this.#t.quiet||bn(s),[r,s]}async#p(e){if(this.#i&&e.data.interceptedAt>this.#i)return e.postMessage("PASSTHROUGH");const r=Bt(e.data);Qr.cache.set(r,r.clone());const s=new vn({event:e,request:r});this.#s.set(e.data.id,s),await this.queue(s)}async#l(e){const{request:r,response:s,isMockedResponse:n}=e.data,o=this.#s.get(r.id);if(s.type?.includes("opaque")){this.#s.delete(r.id),o?.events.removeAllListeners();return}if(this.#s.delete(r.id),o==null)return;const i=Bt(r),a=s.status===0?Response.error():new re(re.isResponseWithBody(s.status)?s.body:null,{...s,url:r.url});try{o.events.emit(new ur(n?"response:mocked":"response:bypass",{requestId:o.data.id,request:i,response:a,isMockedResponse:n}))}finally{o.events.removeAllListeners()}}#c=(e,r)=>e===r;async#d(){const e=new be;return this.#n.postMessage("INTEGRITY_CHECK_REQUEST"),this.#n.once("INTEGRITY_CHECK_RESPONSE",r=>{const{checksum:s,packageVersion:n}=r.data;s!=="4db4a41e972cec1b64cc569c66952d82"&&k.warn(`The currently registered Service Worker has been generated by a different version of MSW (${n}) and may not be fully compatible with the installed version.

It's recommended you update your worker script by running this command:

  • npx msw init <PUBLIC_DIR>

You can also automate this process and make the worker script update automatically upon the library installations. Read more: https://mswjs.io/docs/cli/init.`),e.resolve()}),e}async#u(){if(this.workerPromise.state==="rejected")return;x(this.#a!=null,"[ServiceWorkerSource] Failed to print a start message: client confirmation not received");const e=await this.#a,[r,s]=await this.workerPromise;console.groupCollapsed(`%c${k.formatMessage("Mocking enabled.")}`,"color:orangered;font-weight:bold;"),console.log("%cDocumentation: %chttps://mswjs.io/docs","font-weight:bold","font-weight:normal"),console.log("Found an issue? https://github.com/mswjs/msw/issues"),console.log("Worker script URL:",r.scriptURL),console.log("Worker scope:",s.scope),e&&console.log("Client ID: %s (%s)",e.id,e.frameType),console.groupEnd()}#f(){console.log(`%c${k.formatMessage("Mocking disabled.")}`,"color:orangered;font-weight:bold;")}},vn=class extends Xe{#e;constructor(t){super({request:t.request}),this.#e=t.event}passthrough(){this.#e.postMessage("PASSTHROUGH")}respondWith(t){t&&this.#t(t)}errorWith(t){if(t instanceof Response)return this.respondWith(t);k.warn(`Uncaught exception in the request handler for "%s %s". This exception has been gracefully handled as a 500 response, however, it's strongly recommended to resolve this error, as it indicates a mistake in your code. If you wish to mock an error response, please see this guide: https://mswjs.io/docs/http/mocking-responses/error-responses`,this.data.request.method,this.data.request.url);const e=t instanceof Error?t:new Error(t?.toString()||"Request failure");this.respondWith(l.json({name:e.name,message:e.message,stack:e.stack},{status:500,statusText:"Request Handler Error"}))}async#t(t){let e,r;const s=js(t);sn()?(e=t.body,r=t.body==null?void 0:[t.body]):e=t.body==null?null:await t.clone().arrayBuffer(),this.#e.postMessage("MOCK_RESPONSE",{...s,body:e},r)}},Er=async t=>{try{return{error:null,data:await t().catch(r=>{throw r})}}catch(e){return{error:e,data:null}}};function qr(t,e=!1){return e?Object.prototype.toString.call(t).startsWith("[object "):Object.prototype.toString.call(t)==="[object Object]"}function Be(t,e){try{return t[e],!0}catch{return!1}}function xn(t){return new Response(JSON.stringify(t instanceof Error?{name:t.name,message:t.message,stack:t.stack}:t),{status:500,statusText:"Unhandled Exception",headers:{"Content-Type":"application/json"}})}function Rt(t){return t!=null&&t instanceof Response&&Be(t,"type")&&t.type==="error"}function Sn(t){return qr(t,!0)&&Be(t,"status")&&Be(t,"statusText")&&Be(t,"bodyUsed")}function _n(t){return t==null||!(t instanceof Error)?!1:"code"in t&&"errno"in t}async function Cr(t){const e=async o=>o instanceof Error?(await t.controller.errorWith(o),!0):Rt(o)?(await t.controller.respondWith(o),!0):Sn(o)?(await t.controller.respondWith(o),!0):qr(o)?(await t.controller.errorWith(o),!0):!1,r=async o=>{if(o instanceof $e)throw n.error;return _n(o)?(await t.controller.errorWith(o),!0):o instanceof Response?await e(o):!1},s=new tt;if(t.request.signal){if(t.request.signal.aborted){await t.controller.errorWith(t.request.signal.reason);return}t.request.signal.addEventListener("abort",()=>{s.reject(t.request.signal.reason)},{once:!0})}const n=await Er(async()=>{const o=je(t.emitter,"request",{requestId:t.requestId,request:t.request,controller:t.controller});await Promise.race([s,o,t.controller.handled])});if(s.state==="rejected"){await t.controller.errorWith(s.rejectionReason);return}if(n.error){if(await r(n.error))return;if(t.emitter.listenerCount("unhandledException")>0){const o=new Le(t.request,{passthrough(){},async respondWith(i){await e(i)},async errorWith(i){await t.controller.errorWith(i)}});if(await je(t.emitter,"unhandledException",{error:n.error,request:t.request,requestId:t.requestId,controller:o}),o.readyState!==Le.PENDING)return}await t.controller.respondWith(xn(n.error));return}return t.controller.readyState===Le.PENDING?await t.controller.passthrough():t.controller.handled}function ee(t){return Object.assign(new TypeError("Failed to fetch"),{cause:t})}var Ln=["content-encoding","content-language","content-location","content-type","content-length"],at=Symbol("kRedirectCount");async function En(t,e){if(e.status!==303&&t.body!=null)return Promise.reject(ee());const r=new URL(t.url);let s;try{s=new URL(e.headers.get("location"),t.url)}catch(i){return Promise.reject(ee(i))}if(!(s.protocol==="http:"||s.protocol==="https:"))return Promise.reject(ee("URL scheme must be a HTTP(S) scheme"));if(Reflect.get(t,at)>20)return Promise.reject(ee("redirect count exceeded"));if(Object.defineProperty(t,at,{value:(Reflect.get(t,at)||0)+1}),t.mode==="cors"&&(s.username||s.password)&&!Qt(r,s))return Promise.reject(ee('cross origin not allowed for request mode "cors"'));const n={};([301,302].includes(e.status)&&t.method==="POST"||e.status===303&&!["HEAD","GET"].includes(t.method))&&(n.method="GET",n.body=null,Ln.forEach(i=>{t.headers.delete(i)})),Qt(r,s)||(t.headers.delete("authorization"),t.headers.delete("proxy-authorization"),t.headers.delete("cookie"),t.headers.delete("host")),n.headers=t.headers;const o=await fetch(new Request(s,n));return Object.defineProperty(o,"redirected",{value:!0,configurable:!0}),o}function Qt(t,e){return t.origin===e.origin&&t.origin==="null"||t.protocol===e.protocol&&t.hostname===e.hostname&&t.port===e.port}var qn=class extends TransformStream{constructor(){console.warn("[Interceptors]: Brotli decompression of response streams is not supported in the browser"),super({transform(t,e){e.enqueue(t)}})}},Cn=class extends TransformStream{constructor(t,...e){super({},...e);const r=[super.readable,...t].reduce((s,n)=>s.pipeThrough(n));Object.defineProperty(this,"readable",{get(){return r}})}};function jn(t){return t.toLowerCase().split(",").map(e=>e.trim())}function Rn(t){if(t==="")return null;const e=jn(t);return e.length===0?null:new Cn(e.reduceRight((r,s)=>s==="gzip"||s==="x-gzip"?r.concat(new DecompressionStream("gzip")):s==="deflate"?r.concat(new DecompressionStream("deflate")):s==="br"?r.concat(new qn):(r.length=0,r),[]))}function Pn(t){if(t.body===null)return null;const e=Rn(t.headers.get("content-encoding")||"");return e?(t.body.pipeTo(e.writable),e.readable):null}var Mn=class jr extends Lt{static{this.symbol=Symbol("fetch")}constructor(){super(jr.symbol)}checkEnvironment(){return qt("fetch")}async setup(){const e=globalThis.fetch;x(!e[pe],'Failed to patch the "fetch" module: already patched.'),globalThis.fetch=async(r,s)=>{const n=Et(),o=typeof r=="string"&&typeof location<"u"&&!_r(r)?new URL(r,location.href):r,i=new Request(o,s);r instanceof Request&&Lr(i,r);const a=new tt,u=new Le(i,{passthrough:async()=>{this.logger.info("request has not been handled, passthrough...");const d=i.clone(),{error:m,data:p}=await Er(()=>e(i));if(m)return a.reject(m);if(this.logger.info("original fetch performed",p),this.emitter.listenerCount("response")>0){this.logger.info('emitting the "response" event...');const f=p.clone();await je(this.emitter,"response",{response:f,isMockedResponse:!1,request:d,requestId:n})}a.resolve(p)},respondWith:async d=>{if(Rt(d)){this.logger.info("request has errored!",{response:d}),a.reject(ee(d));return}this.logger.info("received mocked response!",{rawResponse:d});const m=Pn(d),p=m===null?d:new re(m,d);if(re.setUrl(i.url,p),re.isRedirectResponse(p.status)){if(i.redirect==="error"){a.reject(ee("unexpected redirect"));return}if(i.redirect==="follow"){En(i,p).then(f=>{a.resolve(f)},f=>{a.reject(f)});return}}this.emitter.listenerCount("response")>0&&(this.logger.info('emitting the "response" event...'),await je(this.emitter,"response",{response:p.clone(),isMockedResponse:!0,request:i,requestId:n})),a.resolve(p)},errorWith:d=>{this.logger.info("request has been aborted!",{reason:d}),a.reject(d)}});return this.logger.info("[%s] %s",i.method,i.url),this.logger.info("awaiting for the mocked response..."),this.logger.info('emitting the "request" event for %s listener(s)...',this.emitter.listenerCount("request")),await Cr({request:i,requestId:n,emitter:this.emitter,controller:u}),a},Object.defineProperty(globalThis.fetch,pe,{enumerable:!0,configurable:!0,value:!0}),this.subscriptions.push(()=>{Object.defineProperty(globalThis.fetch,pe,{value:void 0}),globalThis.fetch=e,this.logger.info('restored native "globalThis.fetch"!',globalThis.fetch.name)})}};function Tn(t,e){const r=new Uint8Array(t.byteLength+e.byteLength);return r.set(t,0),r.set(e,t.byteLength),r}var Rr=class{constructor(t,e){this.NONE=0,this.CAPTURING_PHASE=1,this.AT_TARGET=2,this.BUBBLING_PHASE=3,this.type="",this.srcElement=null,this.currentTarget=null,this.eventPhase=0,this.isTrusted=!0,this.composed=!1,this.cancelable=!0,this.defaultPrevented=!1,this.bubbles=!0,this.lengthComputable=!0,this.loaded=0,this.total=0,this.cancelBubble=!1,this.returnValue=!0,this.type=t,this.target=e?.target||null,this.currentTarget=e?.currentTarget||null,this.timeStamp=Date.now()}composedPath(){return[]}initEvent(t,e,r){this.type=t,this.bubbles=!!e,this.cancelable=!!r}preventDefault(){this.defaultPrevented=!0}stopPropagation(){}stopImmediatePropagation(){}},On=class extends Rr{constructor(t,e){super(t),this.lengthComputable=e?.lengthComputable||!1,this.composed=e?.composed||!1,this.loaded=e?.loaded||0,this.total=e?.total||0}},In=typeof ProgressEvent<"u";function Nn(t,e,r){const s=["error","progress","loadstart","loadend","load","timeout","abort"],n=In?ProgressEvent:On;return s.includes(e)?new n(e,{lengthComputable:!0,loaded:r?.loaded||0,total:r?.total||0}):new Rr(e,{target:t,currentTarget:t})}function Pr(t,e){if(!(e in t))return null;if(Object.prototype.hasOwnProperty.call(t,e))return t;const r=Reflect.getPrototypeOf(t);return r?Pr(r,e):null}function lt(t,e){return new Proxy(t,Wn(e))}function Wn(t){const{constructorCall:e,methodCall:r,getProperty:s,setProperty:n}=t,o={};return typeof e<"u"&&(o.construct=function(i,a,u){const d=Reflect.construct.bind(null,i,a,u);return e.call(u,a,d)}),o.set=function(i,a,u){const d=()=>{const m=Pr(i,a)||i,p=Reflect.getOwnPropertyDescriptor(m,a);return typeof p?.set<"u"?(p.set.apply(i,[u]),!0):Reflect.defineProperty(m,a,{writable:!0,enumerable:!0,configurable:!0,value:u})};return typeof n<"u"?n.call(i,[a,u],d):d()},o.get=function(i,a,u){const d=()=>i[a],m=typeof s<"u"?s.call(i,[a,u],d):d();return typeof m=="function"?(...p)=>{const f=m.bind(i,...p);return typeof r<"u"?r.call(i,[a,p],f):f()}:m},o}function An(t){return["application/xhtml+xml","application/xml","image/svg+xml","text/html","text/xml"].some(e=>t.startsWith(e))}function Dn(t){try{return JSON.parse(t)}catch{return null}}function Gn(t,e){return new re(re.isResponseWithBody(t.status)?e:null,{url:t.responseURL,status:t.status,statusText:t.statusText,headers:Hn(t.getAllResponseHeaders())})}function Hn(t){const e=new Headers,r=t.split(/[\r\n]+/);for(const s of r){if(s.trim()==="")continue;const[n,...o]=s.split(": "),i=o.join(": ");e.append(n,i)}return e}async function zt(t){const e=t.headers.get("content-length");return e!=null&&e!==""?Number(e):(await t.arrayBuffer()).byteLength}var ke=Symbol("kIsRequestHandled"),Un=_t(),ct=Symbol("kFetchRequest"),$n=class{constructor(t,e){this.initialRequest=t,this.logger=e,this.method="GET",this.url=null,this[ke]=!1,this.events=new Map,this.uploadEvents=new Map,this.requestId=Et(),this.requestHeaders=new Headers,this.responseBuffer=new Uint8Array,this.request=lt(t,{setProperty:([r,s],n)=>{switch(r){case"ontimeout":{const o=r.slice(2);return this.request.addEventListener(o,s),n()}default:return n()}},methodCall:([r,s],n)=>{switch(r){case"open":{const[o,i]=s;return typeof i>"u"?(this.method="GET",this.url=Zt(o)):(this.method=o,this.url=Zt(i)),this.logger=this.logger.extend(`${this.method} ${this.url.href}`),this.logger.info("open",this.method,this.url.href),n()}case"addEventListener":{const[o,i]=s;return this.registerEvent(o,i),this.logger.info("addEventListener",o,i),n()}case"setRequestHeader":{const[o,i]=s;return this.requestHeaders.set(o,i),this.logger.info("setRequestHeader",o,i),n()}case"send":{const[o]=s;this.request.addEventListener("load",()=>{if(typeof this.onResponse<"u"){const u=Gn(this.request,this.request.response);this.onResponse.call(this,{response:u,isMockedResponse:this[ke],request:a,requestId:this.requestId})}});const i=typeof o=="string"?ln(o):o,a=this.toFetchApiRequest(i);this[ct]=a.clone(),queueMicrotask(()=>{(this.onRequest?.call(this,{request:a,requestId:this.requestId})||Promise.resolve()).finally(()=>{if(!this[ke])return this.logger.info("request callback settled but request has not been handled (readystate %d), performing as-is...",this.request.readyState),Un&&this.request.setRequestHeader(zs,this.requestId),n()})});break}default:return n()}}}),ne(this.request,"upload",lt(this.request.upload,{setProperty:([r,s],n)=>{switch(r){case"onloadstart":case"onprogress":case"onaboart":case"onerror":case"onload":case"ontimeout":case"onloadend":{const o=r.slice(2);this.registerUploadEvent(o,s)}}return n()},methodCall:([r,s],n)=>{switch(r){case"addEventListener":{const[o,i]=s;return this.registerUploadEvent(o,i),this.logger.info("upload.addEventListener",o,i),n()}}}}))}registerEvent(t,e){const r=(this.events.get(t)||[]).concat(e);this.events.set(t,r),this.logger.info('registered event "%s"',t,e)}registerUploadEvent(t,e){const r=(this.uploadEvents.get(t)||[]).concat(e);this.uploadEvents.set(t,r),this.logger.info('registered upload event "%s"',t,e)}async respondWith(t){if(this[ke]=!0,this[ct]){const s=await zt(this[ct]);this.trigger("loadstart",this.request.upload,{loaded:0,total:s}),this.trigger("progress",this.request.upload,{loaded:s,total:s}),this.trigger("load",this.request.upload,{loaded:s,total:s}),this.trigger("loadend",this.request.upload,{loaded:s,total:s})}this.logger.info("responding with a mocked response: %d %s",t.status,t.statusText),ne(this.request,"status",t.status),ne(this.request,"statusText",t.statusText),ne(this.request,"responseURL",this.url.href),this.request.getResponseHeader=new Proxy(this.request.getResponseHeader,{apply:(s,n,o)=>{if(this.logger.info("getResponseHeader",o[0]),this.request.readyState<this.request.HEADERS_RECEIVED)return this.logger.info("headers not received yet, returning null"),null;const i=t.headers.get(o[0]);return this.logger.info('resolved response header "%s" to',o[0],i),i}}),this.request.getAllResponseHeaders=new Proxy(this.request.getAllResponseHeaders,{apply:()=>{if(this.logger.info("getAllResponseHeaders"),this.request.readyState<this.request.HEADERS_RECEIVED)return this.logger.info("headers not received yet, returning empty string"),"";const s=Array.from(t.headers.entries()).map(([n,o])=>`${n}: ${o}`).join(`\r
`);return this.logger.info("resolved all response headers to",s),s}}),Object.defineProperties(this.request,{response:{enumerable:!0,configurable:!1,get:()=>this.response},responseText:{enumerable:!0,configurable:!1,get:()=>this.responseText},responseXML:{enumerable:!0,configurable:!1,get:()=>this.responseXML}});const e=await zt(t.clone());this.logger.info("calculated response body length",e),this.trigger("loadstart",this.request,{loaded:0,total:e}),this.setReadyState(this.request.HEADERS_RECEIVED),this.setReadyState(this.request.LOADING);const r=()=>{this.logger.info("finalizing the mocked response..."),this.setReadyState(this.request.DONE),this.trigger("load",this.request,{loaded:this.responseBuffer.byteLength,total:e}),this.trigger("loadend",this.request,{loaded:this.responseBuffer.byteLength,total:e})};if(t.body){this.logger.info("mocked response has body, streaming...");const s=t.body.getReader(),n=async()=>{const{value:o,done:i}=await s.read();if(i){this.logger.info("response body stream done!"),r();return}o&&(this.logger.info("read response body chunk:",o),this.responseBuffer=Tn(this.responseBuffer,o),this.trigger("progress",this.request,{loaded:this.responseBuffer.byteLength,total:e})),n()};n()}else r()}responseBufferToText(){return cn(this.responseBuffer)}get response(){if(this.logger.info("getResponse (responseType: %s)",this.request.responseType),this.request.readyState!==this.request.DONE)return null;switch(this.request.responseType){case"json":{const t=Dn(this.responseBufferToText());return this.logger.info("resolved response JSON",t),t}case"arraybuffer":{const t=dn(this.responseBuffer);return this.logger.info("resolved response ArrayBuffer",t),t}case"blob":{const t=this.request.getResponseHeader("Content-Type")||"text/plain",e=new Blob([this.responseBufferToText()],{type:t});return this.logger.info("resolved response Blob (mime type: %s)",e,t),e}default:{const t=this.responseBufferToText();return this.logger.info('resolving "%s" response type as text',this.request.responseType,t),t}}}get responseText(){if(x(this.request.responseType===""||this.request.responseType==="text","InvalidStateError: The object is in invalid state."),this.request.readyState!==this.request.LOADING&&this.request.readyState!==this.request.DONE)return"";const t=this.responseBufferToText();return this.logger.info('getResponseText: "%s"',t),t}get responseXML(){if(x(this.request.responseType===""||this.request.responseType==="document","InvalidStateError: The object is in invalid state."),this.request.readyState!==this.request.DONE)return null;const t=this.request.getResponseHeader("Content-Type")||"";return typeof DOMParser>"u"?(console.warn("Cannot retrieve XMLHttpRequest response body as XML: DOMParser is not defined. You are likely using an environment that is not browser or does not polyfill browser globals correctly."),null):An(t)?new DOMParser().parseFromString(this.responseBufferToText(),t):null}errorWith(t){this[ke]=!0,this.logger.info("responding with an error"),this.setReadyState(this.request.DONE),this.trigger("error",this.request),this.trigger("loadend",this.request)}setReadyState(t){if(this.logger.info("setReadyState: %d -> %d",this.request.readyState,t),this.request.readyState===t){this.logger.info("ready state identical, skipping transition...");return}ne(this.request,"readyState",t),this.logger.info("set readyState to: %d",t),t!==this.request.UNSENT&&(this.logger.info('triggering "readystatechange" event...'),this.trigger("readystatechange",this.request))}trigger(t,e,r){const s=e[`on${t}`],n=Nn(e,t,r);this.logger.info('trigger "%s"',t,r||""),typeof s=="function"&&(this.logger.info('found a direct "%s" callback, calling...',t),s.call(e,n));const o=e instanceof XMLHttpRequestUpload?this.uploadEvents:this.events;for(const[i,a]of o)i===t&&(this.logger.info('found %d listener(s) for "%s" event, calling...',a.length,t),a.forEach(u=>u.call(e,n)))}toFetchApiRequest(t){this.logger.info("converting request to a Fetch API Request...");const e=t instanceof Document?t.documentElement.innerText:t,r=new Request(this.url.href,{method:this.method,headers:this.requestHeaders,credentials:this.request.withCredentials?"include":"same-origin",body:["GET","HEAD"].includes(this.method.toUpperCase())?null:e});return ne(r,"headers",lt(r.headers,{methodCall:([s,n],o)=>{switch(s){case"append":case"set":{const[i,a]=n;this.request.setRequestHeader(i,a);break}case"delete":{const[i]=n;console.warn(`XMLHttpRequest: Cannot remove a "${i}" header from the Fetch API representation of the "${r.method} ${r.url}" request. XMLHttpRequest headers cannot be removed.`);break}}return o()}})),Lr(r,this.request),this.logger.info("converted request to a Fetch API Request!",r),r}};function Zt(t){return typeof location>"u"?new URL(t):new URL(t.toString(),location.href)}function ne(t,e,r){Reflect.defineProperty(t,e,{writable:!0,enumerable:!0,value:r})}function Bn({emitter:t,logger:e}){return new Proxy(globalThis.XMLHttpRequest,{construct(r,s,n){e.info("constructed new XMLHttpRequest");const o=Reflect.construct(r,s,n),i=Object.getOwnPropertyDescriptors(r.prototype);for(const u in i)Reflect.defineProperty(o,u,i[u]);const a=new $n(o,e);return a.onRequest=async function({request:u,requestId:d}){const m=new Le(u,{passthrough:()=>{this.logger.info("no mocked response received, performing request as-is...")},respondWith:async p=>{if(Rt(p)){this.errorWith(new TypeError("Network error"));return}await this.respondWith(p)},errorWith:p=>{this.logger.info("request errored!",{error:p}),p instanceof Error&&this.errorWith(p)}});this.logger.info("awaiting mocked response..."),this.logger.info('emitting the "request" event for %s listener(s)...',t.listenerCount("request")),await Cr({request:u,requestId:d,controller:m,emitter:t})},a.onResponse=async function({response:u,isMockedResponse:d,request:m,requestId:p}){this.logger.info('emitting the "response" event for %s listener(s)...',t.listenerCount("response")),t.emit("response",{response:u,isMockedResponse:d,request:m,requestId:p})},a.request}})}var Fn=class Mr extends Lt{static{this.interceptorSymbol=Symbol("xhr")}constructor(){super(Mr.interceptorSymbol)}checkEnvironment(){return qt("XMLHttpRequest")}setup(){const e=this.logger.extend("setup");e.info('patching "XMLHttpRequest" module...');const r=globalThis.XMLHttpRequest;x(!r[pe],'Failed to patch the "XMLHttpRequest" module: already patched.'),globalThis.XMLHttpRequest=Bn({emitter:this.emitter,logger:this.logger}),e.info('native "XMLHttpRequest" module patched!',globalThis.XMLHttpRequest.name),Object.defineProperty(globalThis.XMLHttpRequest,pe,{enumerable:!0,configurable:!0,value:!0}),this.subscriptions.push(()=>{Object.defineProperty(globalThis.XMLHttpRequest,pe,{value:void 0}),globalThis.XMLHttpRequest=r,e.info('native "XMLHttpRequest" module restored!',globalThis.XMLHttpRequest.name)})}},Qn=class extends mr{constructor(t){super({interceptors:[new Fn,new Mn]}),this.options=t}enable(){super.enable(),this.options.quiet||this.#e()}disable(){super.disable(),this.options.quiet||this.#t()}#e(){console.groupCollapsed(`%c${k.formatMessage("Mocking enabled (fallback mode).")}`,"color:orangered;font-weight:bold;"),console.log("%cDocumentation: %chttps://mswjs.io/docs","font-weight:bold","font-weight:normal"),console.log("Found an issue? https://github.com/mswjs/msw/issues"),console.groupEnd()}#t(){console.log(`%c${k.formatMessage("Mocking disabled.")}`,"color:orangered;font-weight:bold;")}},zn="/mockServiceWorker.js";function Zn(...t){x(!_t(),k.formatMessage("Failed to execute `setupWorker` in a non-browser environment"));const e=fs({sources:[],handlers:t});return{async start(r){if(r?.waitUntilReady!=null&&k.warn('The "waitUntilReady" option has been deprecated. Please remove it from this "worker.start()" call. Follow the recommended Browser integration (https://mswjs.io/docs/integrations/browser) to eliminate any race conditions between the Service Worker registration and any requests made by your application on initial render.'),e.readyState===yt.ENABLED){k.warn('Found a redundant "worker.start()" call. Note that starting the worker while mocking is already enabled will have no effect. Consider removing this "worker.start()" call.');return}const s=jt()?await Ft.from({serviceWorker:{url:r?.serviceWorker?.url?.toString()||zn,options:r?.serviceWorker?.options},findWorker:r?.findWorker,quiet:r?.quiet}):new Qn({quiet:r?.quiet});if(e.configure({sources:[s,new mr({interceptors:[new rn]})],onUnhandledFrame:Cs(()=>r?.onUnhandledRequest||"warn"),context:{quiet:r?.quiet}}),await e.enable(),s instanceof Ft){const[,n]=await s.workerPromise;return n}},stop(){if(e.readyState===yt.DISABLED){k.warn('Found a redundant "worker.stop()" call. Notice that stopping the worker after it has already been stopped has no effect. Consider removing this "worker.stop()" call.');return}e.disable(),window.postMessage({type:"msw/worker:stop"})},events:e.events,use:e.use.bind(e),resetHandlers:e.resetHandlers.bind(e),restoreHandlers:e.restoreHandlers.bind(e),listHandlers:e.listHandlers.bind(e)}}const Yn="Jeff Mahlas",Yt={"mana.md":`# Mana — the builder
...
`},Jt=[{name:"profile.md",relPath:"00_Core/profile.md",group:"Profile",content:`# Jeff — profile
...
`,ht="data:image/svg+xml,%3csvg%20xmlns='http://www.w3.org/2000/svg'%20viewBox='0%200%2036%2036'%3e%3cpath%20fill='%23C1694F'%20d='M31.595%2015.007c-1.75-3.623-5.934-9.053-9.531-9.053-4.071%200-8.228%207.259-10.071%2010.378-.176.299-.321.578-.464.857-3.371-1.182-.536-6.631-.536-10.463%200-.957-.138-1.637-.44-2.119.489-.606.586-1.347.192-1.699-.413-.367-1.195-.163-1.745.456-.08.089-.129.186-.189.28-.424-.067-.903-.102-1.472-.102-.565%200-2.916.266-4.229.791C-.007%205.582.993%209%201.993%209h4c1%200%20.756%202.31%200%204.726-.83%202.654-1.439%205.145-1%206.606.808%202.687%203.712%203.589%206.164%203.86%201.059%202.659%201.517%206.455%201.473%207.962-.059%202%201.94%202.059%201.999.059.036-1.211-.102-3.68.143-5.781.658%202.833.935%206.097.899%207.314-.059%201.998%201.94%202.057%201.999.059.047-1.602-.182-6.36.559-8.982.507.017%201.044.03%201.619.035%201.774.09%203.726.085%205.506-.015%201.05%201.592%201.996%202.991%201.982%203.435-.029%201-1.117%203.969-1.146%204.969-.029%201%20.94%202.029%201.999.059.648-1.205%201.324-3.419%201.536-5.421.171.364.274.656.269.843-.029%201-.97%203.93-.999%204.93-.029.998.941%202.027%201.999.059%201.059-1.971%201.998-4.898%201.058-6.928-.797-1.72.431-4.165.824-7.914%201.082%201.665%201.117%203.351%201.118%203.459%200%20.553.447%201%201%201%20.553%200%201-.447%201-1-.001-.215-.067-4.85-4.399-7.327z'/%3e%3cpath%20fill='%23292F33'%20d='M8.28%205.571c-.016.552-.477.986-1.029.97-.552-.016-.986-.477-.97-1.029.016-.552.477-.986%201.029-.97.552.016.986.477.97%201.029z'/%3e%3c/svg%3e",rr="/assets/aurora-fjord-img-DAHDArrG.webp",lo=`<div>
  ...
  <div class="rounded-lg overflow-hidden border border-line">
    ...
    <g data-obj="image-2" transform="matrix(0.476,0,0,0.476,583.156,-35.713)"><image href="${rr}" xlink:href="${rr}" x="396.7" y="155" width="206.59" height="310" preserveAspectRatio="xMidYMid meet"></image></g></svg>
  </div>
